...
static int ensure_region_depth_arrays(bedcov_region_t *region) {
    ...
    nbytes = (size_t)region->len * sizeof(uint32_t);
...
static void destroy_bam_bed_cov_bind(void *data) {
    ...
    if (bind->regions) {
        for (i = 0; i < bind->n_regions; i++) {
            ...
            free(bind->regions[i].depth_pre);
            free(bind->regions[i].depth_post);
            free(bind->regions[i].depth_fwd_post);
            free(bind->regions[i].depth_rev_post);
        }
        free(bind->regions);
    }
    duckdb_free(bind);
...
static void accumulate_record_on_region(const bam1_t *rec, const bam_bed_cov_bind_t *bind,
        ...
        if (op == BAM_CMATCH || op == BAM_CEQUAL || op == BAM_CDIFF) {
            ...
            if (clip_end > clip_start) {
                ...
                for (k = clip_start; k < clip_end; k++, query_off++) {
                    int64_t local_idx = k - region->start;
                    increment_depth_cap(region->depth_pre, local_idx, bind->max_depth);
                    ...
                    if (post_ok) {
                        ...
                        if (baseq_ok) {
                            increment_depth_cap(region->depth_post, local_idx, bind->max_depth);
                            if (is_rev) increment_depth_cap(region->depth_rev_post, local_idx, bind->max_depth);
                            else increment_depth_cap(region->depth_fwd_post, local_idx, bind->max_depth);
                            if (has_qual && query_off >= 0 && query_off < rec->core.l_qseq) {
...
static int run_bam_bed_coverage(bam_bed_cov_bind_t *bind, char *err, size_t errlen) {
    ...
    for (i = 0; i < bind->n_regions; i++) {
        ...
        while (sam_itr_next(fp, itr, rec) >= 0) {
            accumulate_record_on_region(rec, bind, region);
        }
        hts_itr_destroy(itr);
    }
...
static void bam_bed_cov_bind(duckdb_bind_info info) {
    ...
    if (bind->mapq < 0 || bind->min_baseq < 0 || bind->min_read_len < 0 ||
        ...
        bind->max_depth < 0 || bind->processing_threads < 0) {
        destroy_bam_bed_cov_bind(bind);
        duckdb_bind_set_error(info, "duckhts_bam_bed_coverage: numeric parameters must be non-negative");
        return;
    }
...
static void bam_bed_cov_scan(duckdb_function_info info, duckdb_data_chunk output) {
    ...
    while (bind->emitted < bind->n_regions && out_idx < max_rows) {
        ...
        int64_t j;
        uint64_t covbases_pre = 0, covbases_post = 0, covbases_fwd_post = 0, covbases_rev_post = 0;
        uint64_t summed_cov_pre = 0, summed_cov_post = 0, summed_cov_fwd_post = 0, summed_cov_rev_post = 0;
        ...
        for (j = 0; j < r->len; j++) {
            uint32_t dpre = r->depth_pre[j];
            uint32_t dpost = r->depth_post[j];
            uint32_t dfwd = r->depth_fwd_post[j];
            uint32_t drev = r->depth_rev_post[j];
            ...
            if (dpost >= (uint32_t)bind->min_depth) {
                covbases_post++;
                summed_cov_post += dpost;
            }
            if (dfwd >= (uint32_t)bind->min_depth) {
                covbases_fwd_post++;
                summed_cov_fwd_post += dfwd;
            }
            if (drev >= (uint32_t)bind->min_depth) {
                covbases_rev_post++;
                summed_cov_rev_post += drev;
            }
        }
        ...
        covbases_pre_data[out_idx] = (int64_t)covbases_pre;
        emit_double_or_null(coverage_pre_vec, out_idx, interval_len > 0 ? (100.0 * (double)covbases_pre / interval_len) : 0.0, interval_len <= 0);
        emit_double_or_null(meandepth_pre_vec, out_idx, interval_len > 0 ? ((double)summed_cov_pre / interval_len) : 0.0, interval_len <= 0);
        ...
        covbases_post_data[out_idx] = (int64_t)covbases_post;
        emit_double_or_null(coverage_post_vec, out_idx, interval_len > 0 ? (100.0 * (double)covbases_post / interval_len) : 0.0, interval_len <= 0);
        emit_double_or_null(meandepth_post_vec, out_idx, interval_len > 0 ? ((double)summed_cov_post / interval_len) : 0.0, interval_len <= 0);
        ...
        covbases_fwd_post_data[out_idx] = (int64_t)covbases_fwd_post;
        covbases_rev_post_data[out_idx] = (int64_t)covbases_rev_post;
        emit_double_or_null(coverage_fwd_post_vec, out_idx, interval_len > 0 ? (100.0 * (double)covbases_fwd_post / interval_len) : 0.0, interval_len <= 0);
        emit_double_or_null(coverage_rev_post_vec, out_idx, interval_len > 0 ? (100.0 * (double)covbases_rev_post / interval_len) : 0.0, interval_len <= 0);
        emit_double_or_null(meandepth_fwd_post_vec, out_idx, interval_len > 0 ? ((double)summed_cov_fwd_post / interval_len) : 0.0, interval_len <= 0);
        emit_double_or_null(meandepth_rev_post_vec, out_idx, interval_len > 0 ? ((double)summed_cov_rev_post / interval_len) : 0.0, interval_len <= 0);
