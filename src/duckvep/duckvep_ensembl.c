...
duckvep_register_ensembl_transcripts(duckdb_connection connection)
{
	static const char *const sql[] = {
		...
		"(CASE WHEN coalesce(tla.rna_edit, false) THEN 256 ELSE 0 END) + ",
		...
		"nullif(concat_ws(',', ",
		"CASE WHEN coalesce(tla.rna_edit, false) THEN 'rna_edit' END, ",
...
duckvep_register_model_receipt(duckdb_connection connection)
{
	static const char *const sql[] = {
		...
		"ELSE true END AS valid), ",
		"region_fingerprint AS MATERIALIZED (SELECT sha256(string_agg(sha256(CAST(to_json(struct_pack(",
		...
		"row_hashes AS MATERIALIZED (SELECT transcript_index, sha256(CAST(to_json(struct_pack(",
		...
		"FROM hash_blocks), provenance_fingerprint AS MATERIALIZED (SELECT sha256(CAST(to_json(struct_pack(",
