...
duckvep_register_annotate_relation(duckdb_connection connection)
{
	static const char *const sql[] = {
		...
		"hgvs := false, upstream_distance := 5000, downstream_distance := 5000) AS TABLE ",
		"WITH parameters AS MATERIALIZED (SELECT CAST(model_name AS VARCHAR) AS model_name, ",
		"CAST(hgvs AS BOOLEAN) AS include_hgvs, ",
		...
		"FROM small_events e CROSS JOIN parameters p WHERE NOT p.include_hgvs), ",
		"small_hgvs_raw AS (SELECT e.*, ",
		"unnest(_duckvep_annotate_small_hgvs(p.model_name, __duckvep_seq_region, ",
		"__duckvep_position, __duckvep_reference, __duckvep_alternate, ",
		"p.upstream_distance, p.downstream_distance)) AS annotation ",
		"FROM small_events e CROSS JOIN parameters p WHERE p.include_hgvs), ",
		"structural_raw AS (SELECT e.*, ",
		...
		"FROM structural_events e CROSS JOIN parameters p), ",
		"breakend_raw AS (SELECT e.*, ",
		...
		"p.upstream_distance, p.downstream_distance)) AS annotation ",
		"FROM breakend_events e CROSS JOIN parameters p), ",
		"small_compact AS (SELECT __duckvep_event_index AS event_index, ",
		"__duckvep_event_kind AS duckvep_event_kind, ",
		"annotation.*, NULL::VARCHAR AS transcript_hgvs, NULL::VARCHAR AS protein_hgvs, ",
		"NULL::UINTEGER AS hgvs_shift, NULL::VARCHAR AS transcript_hgvs_status, ",
		"NULL::VARCHAR AS transcript_hgvs_reason, NULL::VARCHAR AS protein_hgvs_status, ",
		"NULL::VARCHAR AS protein_hgvs_reason FROM small_compact_raw), ",
		"small_hgvs AS (SELECT __duckvep_event_index AS event_index, ",
		"__duckvep_event_kind AS duckvep_event_kind, ",
		"annotation.* FROM small_hgvs_raw), ",
		"structural AS (SELECT __duckvep_event_index AS event_index, ",
		"__duckvep_event_kind AS duckvep_event_kind, ",
		"annotation.*, NULL::VARCHAR AS transcript_hgvs, NULL::VARCHAR AS protein_hgvs, ",
		"NULL::UINTEGER AS hgvs_shift, NULL::VARCHAR AS transcript_hgvs_status, ",
		"NULL::VARCHAR AS transcript_hgvs_reason, NULL::VARCHAR AS protein_hgvs_status, ",
		"NULL::VARCHAR AS protein_hgvs_reason FROM structural_raw), ",
		"breakends AS (SELECT __duckvep_event_index AS event_index, ",
		"__duckvep_event_kind AS duckvep_event_kind, ",
		"annotation.*, NULL::VARCHAR AS transcript_hgvs, NULL::VARCHAR AS protein_hgvs, ",
		"NULL::UINTEGER AS hgvs_shift, NULL::VARCHAR AS transcript_hgvs_status, ",
		"NULL::VARCHAR AS transcript_hgvs_reason, NULL::VARCHAR AS protein_hgvs_status, ",
		"NULL::VARCHAR AS protein_hgvs_reason FROM breakend_raw) ",
		"SELECT * FROM small_compact UNION ALL BY NAME SELECT * FROM small_hgvs ",
		"UNION ALL BY NAME SELECT * FROM structural UNION ALL BY NAME SELECT * FROM breakends"
	};
