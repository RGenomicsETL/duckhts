...
DUCKDB_EXTENSION_ENTRYPOINT(duckdb_connection connection,
    ...
    }
    {
        static const char *const duckhts_bcftools_norm_sql[] = {
            ...
            "'UNNEST(duckhts_alt_to_list(src.__duckhts_alt)) WITH ORDINALITY AS u0(alt_item, alt_index), ' || ",
            "'LATERAL (SELECT bcftools_norm_row(src.__duckhts_chrom, src.__duckhts_pos, src.__duckhts_ref, u0.alt_item, ' || ",
