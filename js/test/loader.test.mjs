...
import { extensionUrl, loadDuckhts, SIGNED } from "../src/index.js";

function recordingConnection(platform, unsignedAllowed = true) {
  const statements = [];
  return {
    statements,
    async query(sql) {
      ...
      return { toArray: () => (sql === "PRAGMA platform" ? [{ platform }] :
        sql.includes("current_setting('allow_unsigned_extensions')") ? [{ enabled: unsignedAllowed }] : []) };
    },
  };
}
...
test("loadDuckhts loads the binary for the connection's platform", async () => {
  ...
  assert.deepEqual(conn.statements, [
    ...(!SIGNED ? ["SELECT current_setting('allow_unsigned_extensions') AS enabled"] : []),
    "PRAGMA platform",
...
test("dev channel refuses to load when unsigned extensions are disallowed", async () => {
  if (SIGNED) return;
  const conn = recordingConnection("wasm_eh", false);
  await assert.rejects(loadDuckhts(conn, { baseUrl: "https://example.org/" }),
    /dev channel requires allow_unsigned_extensions=true/);
  assert.deepEqual(conn.statements, ["SELECT current_setting('allow_unsigned_extensions') AS enabled"]);
});
