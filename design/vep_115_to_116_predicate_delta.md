...
| 11 | `_ins_del_stop_altered` | `VariationEffect.pm:1382-1383` (115): `sub _ins_del_stop_altered { my ($bvfoa, $feat, $bvfo, $bvf) = @_;` — 4-arg signature; body calls `_overlaps_stop_codon(@_);` (line 1393). | `VariationEffect.pm:1433-1434,1444` (116): `sub _ins_del_stop_altered { my ($bvfoa, $feat, $bvfo, $bvf, $consider_ins_len) = @_;` — new, unused 5th positional parameter `$consider_ins_len`; body calls `_overlaps_stop_codon(@_, $consider_ins_len);` (line 1444). | The rest of the function body is byte-identical between 115 and 116 (verified by direct read of both, `VariationEffect.pm:1382-1433` in 115 vs `1433-1484` in 116). Because `_overlaps_stop_codon` only destructures its first 4 positional args (`my ($bvfoa, $feat, $bvfo, $bvf) = @_;`, unchanged in both versions), the appended `$consider_ins_len` argument is a **no-op today** — it is passed through but never read by the callee. `stop_lost`'s fallback call site (`VariationEffect.pm:1263` in 116, unchanged text `$cache->{stop_lost} = _ins_del_stop_altered(@_);`) still uses this **plain** (non-`_cil`) variant, so `stop_lost`'s indel-near-stop fallback logic is behaviorally unchanged from 115 aside from the dead parameter. | `stop_lost` (fallback path) | Partially — no observable behavior change today, but the added parameter is a forward-looking hook worth flagging for implementers (see UNRESOLVED). |
...
| # | function | 115 behavior (quoted) | 116 behavior (quoted) | semantic change | HGVS impact | M6b frontier? |
|---|---|---|---|---|---|---|
| 1 | `get_3prime_seq_offset` | `Sequence.pm:801-802` (115): `## get length of pattern to check` / `my $check_length = length($post_seq) - length($seq_to_check);` — loop bound is `downstream_flank_length - allele_length`. | `Sequence.pm:801-802` (116): `## get length of pattern to check` / `my $check_length = length($post_seq);` — loop bound is simply `downstream_flank_length`. | `get_3prime_seq_offset($seq_to_check, $post_seq)` walks a `for (my $n = 0; $n <= $check_length; $n++)` loop that, at each step, compares the first character of the (rotating) `$seq_to_check` to `substr($post_seq, $n, 1)`; on a match it rotates `$seq_to_check` (drop first char, append the matched char) and increments `$offset`, stopping at the first mismatch (`Sequence.pm:804-820`, unchanged in both versions). In 115 the walk is artificially capped at `length($post_seq) - length($seq_to_check)` iterations even though each iteration only ever consumes a single character of `$post_seq` (not `length($seq_to_check)` characters) — so for a deleted/inserted allele sitting in front of a long homopolymer/tandem-repeat 3' flank, 115 can stop shifting early (return a smaller `$offset`, i.e. a **less-3'-shifted** position) purely because the artificial cap was reached, even though the sequence still matches further into `$post_seq`. In 116 the cap is simply the full downstream flank length, so the walk can continue until an actual sequence mismatch is found — the true maximal 3' shift for however much flank the caller fetched. Confirmed caller: `VariationFeature.pm:2003` (both versions, path unchanged) — `($three_prime_allele, $offset) = get_3prime_seq_offset($seq_to_check, $downstream_seq);` where `$downstream_seq = substr($ref_seq, $ref_end)` (`VariationFeature.pm:1996` area) feeds the genomic `g.` HGVS 3' shift for insertions/deletions. | Genomic (`g.`) HGVS 3' shift/offset for indels can differ between 115 and 116 whenever the true maximal repeat-shift distance exceeds `downstream_flank_length - allele_length`: 115 under-shifts (reports an earlier/less-3' position and a smaller offset) in that case; 116 shifts as far as the fetched flank allows, which is the HGVS-correct "shift 3' until a mismatch" behavior. `get_3prime_seq_offset` is also imported (but its call is not directly visible under this name) by `TranscriptVariationAllele.pm` (`use ... qw(hgvs_variant_notation format_hgvs_string get_3prime_seq_offset)` at `TranscriptVariationAllele.pm:76`, unchanged in both versions), so the same shift routine is in scope for transcript-level HGVS as well as genomic. | Yes — this is exactly the "3'-shift / alignment logic" the task brief flagged, and it changes HGVS normalization/offset for indels in repetitive downstream context. |
...
| # | function/area | 115 behavior (quoted) | 116 behavior (quoted) | semantic change | c./n. coordinate impact | M6b frontier? |
|---|---|---|---|---|---|---|
...
6. **HGVS 3' shifting (`get_3prime_seq_offset`) must not cap its walk at `flank_length - allele_length`.** A
   115-ported implementation will under-shift indels sitting in front of long homopolymer/repeat downstream
   context, producing a less-3' (and therefore non-HGVS-canonical) position/offset than 116. This is a
   genomic-level (`g.`) concern per the confirmed caller in `VariationFeature.pm:2003`; the same routine is in
   scope for transcript-level HGVS via `TranscriptVariationAllele.pm`'s import, though no direct in-file call was
   found there (see UNRESOLVED #2 below).
