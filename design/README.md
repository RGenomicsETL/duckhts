...
| [`roadmap.md`](roadmap.md) | apex | Future proposal. Converged M1–M7 milestone sequence (QC/coverage foundations → riker/RustQC parity → DuckVEP fuse → gated minibwa) with per-milestone oracle pins, the `FILE_OFFSET`-sort order model, and the primitive verdicts. Elaborates `north_star.md` sequencing. | Milestone order, upstream oracle pins, QC-stack primitives, coverage memory/order model, minibwa gate. |
| [`duckvep_reconciliation.md`](duckvep_reconciliation.md) | L3 | Future proposal (partly superseded by `duckvep_m6a_contract_gate.md` on the edit model + VEP pin). Salvage plan for folding `/root/duckvep-c` (`9f922c8`) into `src/duckvep/` before archive: the three-tier conformance framework (the anti-slop differentiator vs fastVEP), the Model A transcript-import pivot, format-as-relations, salvage inventory, and the ERRATA+manifest discipline. | DuckVEP fuse (roadmap M6), consequence-engine conformance, Model A/MySQL-dump import, selenocysteine/miRNA/MANE provenance, ERRATA discipline. |
| [`duckvep_m6a_contract_gate.md`](duckvep_m6a_contract_gate.md) | L3 | Open design/implementation contract. The M6a "contract gate" before folding duckvep-c: verified gpt-5.6-sol (max) findings — SO/HGVS as sibling consumers of a lossless `allele_transcript_context`, Model A v2 typed edit relations (nucleotide-pre / peptide-post translation), VEP-116 pin (resolved), two-lane oracle, conformance-must-gate, `duckhgvs-0.4.0` quarantine, human-first codon-table scope. **Authoritative over the reconciliation doc where they disagree.** | duckvep/duckhgvs sequencing (roadmap M6a–M6d), Model A v2 schema, the lossless intermediate, VEP version pin, HGVS oracle policy, 3′-shift vs left-align, conformance gating. |
...
| [`duckvep_layer_keys.md`](duckvep_layer_keys.md) | L3 | Future DuckVEP/keying design. | VariantKey/RegionKey annotation joins, VEP-style consequence-layer planning, interval/exact annotation tables. |
| [`duckvep_bcftools_csq_port_plan_2026-06-09.md`](duckvep_bcftools_csq_port_plan_2026-06-09.md) | L3 | Open design/backlog. Phased DuckVEP + `bcftools csq` port plan, upstream refs, DuckDB-native storage stance, HGVS/testing/performance roadmap, and TigerStyle guardrails. | DuckVEP consequence-engine work, `bcftools csq` compatibility planning, HGVS architecture, VEP/fastVEP study, GIAB/performance test plumbing. |
