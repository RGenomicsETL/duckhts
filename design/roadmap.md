...
Status: **future proposal / open design.** Work is ordered by dependency; each stage names
its enabling primitive, largest risk, and conformance oracle. This is direction, not a
contract; update it as implementations land or source pins change.
...
- **No headline parity-% while backlog remains.** Coverage is a counted, per-class
  number backed by the accepted-mismatch ledger, never a marketing figure.
...
| Ensembl core annotation schema | [Ensembl/ensembl `sql/table.sql`](https://github.com/Ensembl/ensembl/blob/c0cf13daa961d80584bad797b2eb0ff3a7500ef3/sql/table.sql) | `c0cf13daa961d80584bad797b2eb0ff3a7500ef3` |
...
| ACMG SQL engine reference | `/root/bioconnect-acmg/manifests/*.sql` | — |

**Assemblies and species:** GRCh38 **and** GRCh37 are first-class — GRCh37/hg19
is clinically load-bearing (Ensembl archive DB / VEP `--grch37` / GENCODE lift37,
FASTA `/root/GRCh37/human_g1k_v37.fasta`). The annotation model is assembly-parameterized and
...
### Freeze contracts, oracle pins, and parity matrices
...
and an **accepted-mismatch ledger** (manifest = *which* oracle, errata = *why* we differ
...
### Shared typed CIGAR/aligned-pair walker
...
### Ordered streaming infrastructure
...
### DNA QC parity, then a fused `duckhts_riker` pass
...
### RNA and RustQC analytical parity
...
### DuckVEP consequence and DuckHGVS

The binding contract is `design/duckvep_model.md`. Implement in this order: normalized
annotation import and five sequence states; lossless allele/transcript context; a gated
VEP-116 consequence slice; DNA HGVS; then protein, compound, mitochondrial, haplotype, and
structural HGVS as separate capabilities. Consequence and HGVS are sibling consumers of the
context and never derive one from the other.

Scope the first claim to **human GRCh37/38** (kernel supports only codon tables 1/2).
Risk: normalized annotation import and the lossless intermediate, not the state machine.
...
### Clinical SQL layer: configurable ACMG/AMP engine
Sits on consequence output plus supplementary-annotation Parquet/DuckLake joins
(gnomAD / dbNSFP-REVEL / SpliceAI / ClinVar). The classification is *already pure
SQL* — a direct validation of the SQL-composition thesis. See the reference engine
...
sources pinned above (Pejaver 2022 REVEL, Walker 2023 SpliceAI).

### Optional minibwa `seq_align`
Only if the portability spike promotes; never a dependency and never delays the core work.
