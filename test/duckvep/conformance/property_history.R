...
main <- function() {
  ...
  preserve_failure_log <- function() {
    if (
      !isTRUE(campaign_started) || is.null(log_path) || !file.exists(log_path)
    ) {
      return(NULL)
    }
  ...
  if (
    normalizePath(opt$history, mustWork = FALSE) ==
  ...
  restore_interrupted_publication <- function(interrupted_lock_dir) {
    ...
    if (
      !identical(names(journal), expected_journal_names) ||
    ...
    if (
      !identical(dirname(interrupted_history), history_dir) ||
  ...
  lock_owner_state <- function(candidate_lock_dir) {
    ...
    owner_alive <- length(owner_pid) == 1L &&
      !is.na(owner_pid) &&
      isTRUE(tryCatch(
  ...
  recover_stale_publication <- function() {
    ...
    if (dir.exists(recovery_dir)) {
      ...
      if (recovery_state$alive) {
        die(
          "property-history recovery is active in process {recovery_state$pid}"
        )
      }
      if (
        (length(recovery_state$pid) != 1L || is.na(recovery_state$pid)) &&
          (length(recovery_state$age) != 1L ||
            is.na(recovery_state$age) ||
            recovery_state$age < 60)
      ) {
        die(
          "property-history recovery lock is still initializing: {recovery_dir}"
        )
      }
      if (unlink(recovery_dir, recursive = TRUE) != 0L) {
        die(
          "cannot remove stale property-history recovery lock: {recovery_dir}"
        )
      }
    }
    ...
    if (
      (length(owner_state$pid) != 1L || is.na(owner_state$pid)) &&
        (length(owner_state$age) != 1L ||
          is.na(owner_state$age) ||
          owner_state$age < 60)
    ) {
  ...
  if (
    (!is.null(git_head_status) && git_head_status != 0L) ||
      length(git_head) != 1L
  ) {
  ...
  check_source_tree <- function(expected_head) {
    ...
    if (
      (!is.null(current_head_status) && current_head_status != 0L) ||
        length(current_head) != 1L ||
        trimws(current_head[[1L]]) != expected_head
    ) {
  ...
  if (!file.exists(opt$coverage_requirements)) {
    die(
      "coverage requirement manifest does not exist: {opt$coverage_requirements}"
    )
  }
  ...
  if (
    !nrow(requirements) ||
      anyNA(requirements) ||
      any(!nzchar(as.matrix(requirements)))
  ) {
  ...
  if (anyNA(requirement_index)) {
    missing <- paste(
      paste0(
        requirements$coverage[is.na(requirement_index)],
        "/",
  ...
  if (any(missing_statistical)) {
    missing <- paste(
      paste0(
        requirements$coverage[missing_statistical],
        "/",
  ...
  if (length(zero_index)) {
    ...
    if (any(!allowed_zero)) {
      missing <- paste(
        paste0(
          coverage$coverage[zero_index[!allowed_zero]],
          "/",
      ...
      die(
        "coverage counters reached zero without a fixed-witness declaration: {missing}"
      )
    }
  }
  ...
  if (
    any(
      requirements$requirement == "statistical_required" &
  ...
  if (any(fixed_requirements)) {
    ...
    for (index in seq_along(evidence)) {
      ...
      if (start[[1L]] < 0L) {
        die(
          "coverage requirement manifest names absent fixed witness: {test_name}"
        )
      }
  ...
  if (
    nrow(total) != 1L ||
      nrow(suite) != 1L ||
      as.integer(suite$suite_failed) != 0L
  ) {
  ...
  publish_histories <- function() {
    ...
    if (
      length(new_run_key) != 1L || !identical(new_run_key, new_coverage_key)
    ) {
      die("property and coverage rows do not describe one identical campaign")
    }
    if (!is.null(old) && new_run_key %in% campaign_key(old)) {
      die(
        "property campaign already exists in immutable history: {new_run_key}"
      )
    }
    if (!is.null(old_coverage) && new_run_key %in% campaign_key(old_coverage)) {
      die(
        "property campaign already exists in immutable coverage history: {new_run_key}"
      )
    }
    if (!is.null(old)) {
      rows <- rbind(old, rows)
    ...
    if (
      !identical(names(staged_properties), names(rows)) ||
