...
main <- function() {
  ...
  for (i in seq_len(nrow(cases))) {
    ...
    if (shape == 'mnv_retained_middle') alts[1L] <- paste0(change(substr(refs[1L],1L,1L)),
      substr(refs[1L],2L,6L),change(substr(refs[1L],7L,7L)))
    ...
    if (shape %in% c('containing_insertion','same_start_length_change'))
      alts[1L] <- paste0(substr(refs[1L],1L,1L),'AC',substring(refs[1L],2L))
    if (shape == 'partial_overlap') alts[1:2] <- c('ACGT','GTC')
    if (shape == 'random_overlap') {
      alts[1:2] <- vapply(sample(1:10,2L,replace=TRUE),dna,'')
      for (j in 1:2) if (alts[j] == refs[j]) alts[j] <- change(alts[j])
    }
    gt <- genotypes[match(cases$phase[i],genotypes$phase),]
  ...
  comparisons <- lapply(seq_len(nrow(cases)),function(i) {
    a <- actual[rows_by_tx[[as.character(cases$transcript_index[i])]],,drop=FALSE]
    observed <- lapply(seq_len(nrow(a)),function(j) {
      ids <- unlist(a$coding_blocks[[j]]$event_indices,use.names=FALSE)
      matched <- match(ids,records$event_index)
    ...
    expected <- oracle[[cases$transcript[i]]]$haplotypes
    list(expected=canonical(expected),observed=canonical(observed),
      ...
      counts_equal=sum(a$carrier_count) == oracle[[cases$transcript[i]]]$total_haplotype_count,
  ...
  witness <- oracle[[cases$transcript[1L]]]$haplotypes
  ...
  stopifnot(all(rejected),all(summary$equal[summary$shape %in% c('disjoint','adjacent')]))
