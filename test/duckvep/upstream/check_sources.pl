...
my @expected_self_test_header = qw(
  ...
  module_distribution_sha256 perl_version files assertions skipped_assertions
  skip_reasons result
);
...
while (my $line = <$self_test>) {
  ...
  next if $line eq '';
  my @field = split /\t/, $line, -1;
  die "$self_test_receipts expected 13 tab-separated columns\n" unless @field == 13;
  my ($run_date, $repository_url, $ref, $commit, $test_selector,
      $module_distribution, $module_sha256, $perl_version, $files,
      $assertions, $skipped, $skip_reasons, $result) = @field;
  my $pin = $authority{$repository_url}
...
die "$self_test_receipts contains no receipt\n" unless $self_test_count;
