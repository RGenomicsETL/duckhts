...
static enum theft_trial_res prop_null_model_rejected(struct theft *t, void *arg1) {
    ...
    s = duckvep_annotate_tile(NULL, &b->view, NULL, NULL, NULL, NULL, &err);
...
TEST null_args_are_rejected(void) {
    ...
    ASSERT_EQ(DUCKVEP_ERR_INVALID_ARG,
              duckvep_annotate_tile(NULL, NULL, NULL, NULL, NULL, NULL, &err));
    PASS();
}
...
TEST event_load_trims_small_variant_differing_region(void) {
    ...
    for (i = 0u; i < 4u; i++) {
        ...
        ASSERT_EQ(exp_end[i], duckvep_event_effective_end1_at(&v, i));
    }
    PASS();
}
...
static uint32_t allele_sweep_oracle_end(const duckvep_variant_batch_t *v, size_t i) {
    ...
    if (ref_diff_len == 0u && alt_diff_len > 0u) {
        return prefix > 0u ? v->pos1[i] + (uint32_t)prefix - 1u : v->pos1[i];
    }
    start = v->pos1[i] + (uint32_t)prefix;
    return ref_diff_len > 0u ? start + (uint32_t)ref_diff_len - 1u : start;
}
...
static enum theft_trial_res prop_allele_sweep_matches_trim_oracle(
    ...
    for (i = 0u; i < s->v.count; i++) {
        ...
        if (event.end1 != allele_sweep_oracle_end(&s->v, i)) return THEFT_TRIAL_FAIL;
...
TEST sweep_allele_normalized_candidates_match_trim_oracle(void) {
    struct theft_run_config cfg;
    memset(&cfg, 0, sizeof cfg);
    cfg.name = "allele-normalized sweep candidates == independent trim oracle";
...
static enum theft_trial_res prop_annotate_matches_composition(struct theft *t, void *arg1) {
    ...
    if (duckvep_annotate_tile(model, &s->v, NULL, opts, ws, &rb, &err) != DUCKVEP_OK) {
...
static int consequence_rows_equal(const duckvep_consequence_t *a,
    ...
    return a->variant_idx == b->variant_idx &&
           ...
           a->aa_ref == b->aa_ref &&
           a->aa_alt == b->aa_alt &&
           a->codon_ref == b->codon_ref &&
           a->codon_alt == b->codon_alt;
}
...
TEST annotate_cursor_resumes_known_scene(void) {
    ...
    ASSERT_EQ(DUCKVEP_OK, duckvep_annotate_cursor_open(model, &v, NULL, opts, ws, &cur, &err));
...
static enum theft_trial_res prop_annotate_cursor_matches_tile(struct theft *t, void *arg1) {
    ...
    if (duckvep_annotate_cursor_open(model, &s->v, NULL, opts, ws, &cur, &err) != DUCKVEP_OK) {
        res = THEFT_TRIAL_FAIL; goto done;
    }
    while (!duckvep_annotate_cursor_done(cur)) {
...
TEST annotate_structural_known_scene(void) {
    ...
    ASSERT_EQ(DUCKVEP_OK, duckvep_annotate_tile(model, &v, NULL, opts, ws, &rb, &err));
...
TEST annotate_sv_cnv_known_scene(void) {
     ...
     * validator must also reject contradictory structural metadata. */
    {
        ...
        ASSERT_EQ(DUCKVEP_ERR_INVALID_ARG,
                  duckvep_annotate_tile(model, &bad, NULL, opts, ws, &rb, &err));
    }
 ...
 * (genomic 1009,1010), cds12 in exon1 (genomic 2000). Two SNVs:
...
TEST annotate_codon_multi_exon_known_scene(void) {
    ...
    static const uint16_t vchrom[2] = {0u, 0u};
    static const uint32_t vpos[2]   = {1004u, 2000u};
    static const uint8_t  vkind[2]  = {0u, 0u};
    static const uint8_t  abytes[4] = {'A','T', 'T','C'}; /* 1004 A>T, 2000 T>C */
    static const uint32_t roff[2]   = {0u, 2u};
    static const uint32_t aoff[2]   = {1u, 3u};
    static const uint16_t rlen[2]   = {1u, 1u};
    static const uint16_t alen[2]   = {1u, 1u};

    static const uint64_t exp_mask[2] = {
        DUCKVEP_SO(DUCKVEP_SO_MISSENSE),
        DUCKVEP_SO(DUCKVEP_SO_SYNONYMOUS) | DUCKVEP_SO(DUCKVEP_SO_SPLICE_REGION)
    };
    static const int32_t exp_cds[2]     = {5, 12};
    static const int32_t exp_protein[2] = {2, 4};
    static const char    exp_aa_ref[2]  = {'K', 'R'};
    static const char    exp_aa_alt[2]  = {'I', 'R'};
...
TEST annotate_codon_mnv_same_codon_known_scene(void) {
    ...
    ASSERT(stats != NULL);
    ASSERT_EQ(1u, stats->mnv_context_accepted);
    ASSERT_EQ(0u, stats->mnv_context_fallback_mismatch);
    ASSERT_EQ(0u, stats->mnv_context_fallback_unsupported);
...
TEST annotate_cursor_mnv_start_lost_route_matches_tile_known_scene(void) {
    ...
    ASSERT_EQ(1u, tile_stats.mnv_context_accepted);
    ASSERT_EQ(0u, tile_stats.mnv_context_fallback_mismatch);
    ASSERT_EQ(0u, tile_stats.mnv_context_fallback_unsupported);
    ASSERT_EQ(tile_stats.mnv_context_accepted, cursor_stats.mnv_context_accepted);
    ASSERT_EQ(tile_stats.mnv_context_fallback_mismatch, cursor_stats.mnv_context_fallback_mismatch);
    ASSERT_EQ(tile_stats.mnv_context_fallback_unsupported, cursor_stats.mnv_context_fallback_unsupported);
...
TEST annotate_codon_mnv_len3_and_cross_codon_known_scene(void) {
    ...
    stats = duckvep_workspace_delta_route_stats(ws);
    ASSERT(stats != NULL);
    ASSERT_EQ(2u, stats->mnv_context_accepted);
    ASSERT_EQ(0u, stats->mnv_context_fallback_mismatch);
    ASSERT_EQ(0u, stats->mnv_context_fallback_unsupported);
...
TEST annotate_codon_mnv_cross_codon_missense_known_scene(void) {
    ...
    stats = duckvep_workspace_delta_route_stats(ws);
    ASSERT(stats != NULL);
    ASSERT_EQ(5u, stats->mnv_context_accepted);
    ASSERT_EQ(0u, stats->mnv_context_fallback_mismatch);
    ASSERT_EQ(0u, stats->mnv_context_fallback_unsupported);
...
TEST annotate_codon_indel_frameshift_known_scene(void) {
    ...
    ASSERT_EQ(DUCKVEP_SO(DUCKVEP_SO_CODING_SEQUENCE), rows[0].consequence_mask); /* start-codon indel pending */
     ...
     * the frameshift the legacy body-only restriction rejected at the terminal codon. */
...
TEST annotate_codon_delins_frameshift_known_scene(void) {
    ...
    static const int32_t exp_protein[4] = {2, 2, -1, 2};
...
TEST annotate_codon_inframe_insertion_known_scene(void) {
    ...
    ASSERT_EQ(-1, rows[0].cds_pos);
    ASSERT_EQ(3, rows[0].protein_pos);
    ASSERT_EQ(DUCKVEP_SO(DUCKVEP_SO_CODING_SEQUENCE), rows[1].consequence_mask); /* inserted stop */
...
TEST annotate_codon_protein_altering_insertion_known_scene(void) {
    ...
    static const uint8_t  abytes[15] = {
        ...
        'A', 'A','C','C','T',  /* + strand: junction creates TAA stop -> fallback */
    ...
    ASSERT_EQ(2, rows[0].protein_pos);
    ASSERT_EQ(DUCKVEP_SO(DUCKVEP_SO_CODING_SEQUENCE), rows[1].consequence_mask);
...
TEST annotate_rejects_prefix_zero_pure_insertion(void) {
    static const uint16_t tchrom[1] = {0u};
    static const uint32_t tstart[1] = {100u};
    static const uint32_t tend[1]   = {200u};
    static const int8_t tstrand[1]  = {1};
...
static enum theft_alloc_res kprop_frameshift_indel_alloc(struct theft *t, void *env, void **instance) {
    ...
    if (mode == 0u) {
        ...
        cds_pos = (uint32_t)kprop_bounded(t, cds_len - 6u) + 4u; /* exclude first/last codon */
...
static enum theft_alloc_res kprop_inframe_delins_alloc(struct theft *t, void *env, void **instance) {
...
static struct theft_type_info kprop_inframe_delins_info = {
    .alloc = kprop_inframe_delins_alloc,
    .free  = kprop_coding_free,
};
...
static enum theft_trial_res prop_annotate_codon_matches_kernel(struct theft *t, void *arg1) {
    ...
    duckvep_result_builder_init(&rb, rows, 2u);
    if (duckvep_annotate_tile(model, &s->v, NULL, opts, ws, &rb, &err) != DUCKVEP_OK) { tr = THEFT_TRIAL_FAIL; goto done; }
...
static enum theft_trial_res prop_annotate_cross_codon_mnv_matches_oracle(struct theft *t, void *arg1) {
    ...
    if (rows[0].consequence_mask != want) { tr = THEFT_TRIAL_FAIL; goto done; }
    if (stats->mnv_context_fallback_mismatch != 0u) { tr = THEFT_TRIAL_FAIL; goto done; }
    if (f.valid) {
        if (stats->mnv_context_accepted != 1u ||
            stats->mnv_context_fallback_unsupported != 0u) {
            tr = THEFT_TRIAL_FAIL; goto done;
        }
    } else {
        if (stats->mnv_context_accepted != 0u ||
            stats->mnv_context_fallback_unsupported != 1u) {
            tr = THEFT_TRIAL_FAIL; goto done;
        }
    }
...
static int kprop_delta_is_frameshift_at(const duckvep_sequence_delta_t *d,
    ...
    return d != NULL && d->valid && d->frameshift &&
           ...
           !d->stop_retained && !d->start_lost && !d->inframe_deletion &&
 ...
 * change not divisible by three, with the start codon preserved, resolves to frameshift;
 * a start-codon-disrupting frameshift is honest backlog (VEP would add start_lost). */
TEST coding_context_delta_frameshift_known_scene(void) {
    ...
    /* (3) +1 frameshift disrupting the start codon (cds 1): honest backlog, no fact. */
...
static int kprop_delta_is_coarse_cross_codon_missense(const duckvep_sequence_delta_t *d) {
    return d != NULL && d->valid && d->missense && !d->synonymous &&
           ...
           !d->start_lost && !d->frameshift && !d->inframe_deletion &&
           !d->inframe_insertion && !d->protein_altering &&
...
static int kprop_delta_is_inframe_deletion_at(const duckvep_sequence_delta_t *d,
    ...
    return d != NULL && d->valid && d->inframe_deletion &&
           !d->synonymous && !d->missense && !d->stop_gained && !d->stop_lost &&
           !d->stop_retained && !d->start_lost && !d->frameshift &&
           !d->inframe_insertion && !d->protein_altering &&
           d->cdna_pos == -1 && d->cds_pos == -1 && d->protein_pos == protein_pos &&
           d->ref_aa == (uint8_t)0u && d->alt_aa == (uint8_t)0u;
}
...
TEST coding_context_delta_inframe_deletion_known_scene(void) {
    ...
    /* In-frame deletion overlapping the start codon (CDS positions 1-3): a start-codon deletion
     * is start_lost territory in VEP, deferred to the start-boundary slice (cds_start <= 3). */
    {
...
TEST coding_context_delta_inframe_delins_known_scene(void) {
    static uint8_t cds[18] = {
    ...
    for (case_idx = 0u; case_idx < 2u; case_idx++) {
        ...
        ASSERT_EQ(DUCKVEP_CONTEXT_DELTA_OK,
                  duckvep_coding_context_delta_fill(&ctx, 0u, &delta));
        ASSERT(kprop_delta_is_inframe_deletion_at(&delta, 3));
    }
...

    {
        ...
        duckvep_workspace_delta_route_stats_reset(ws);
        duckvep_result_builder_init(&rb, rows, 2u);
        ASSERT_EQ(DUCKVEP_OK, duckvep_annotate_tile(model, &s.v, NULL, opts, ws, &rb, &err));
        ASSERT_EQ(1u, duckvep_result_builder_count(&rb));
        /* +3 delins (replace codon 3 with two codons) in the CDS body: the INDEL
         * gap-fill routes it through the general CodingContext, which resolves the
         * net-lengthening in-frame delins as an inframe_insertion. */
        ASSERT_EQ(DUCKVEP_SO(DUCKVEP_SO_INFRAME_INSERTION), rows[0].consequence_mask);
        stats = duckvep_workspace_delta_route_stats(ws);
        ASSERT(stats != NULL);
        ASSERT_EQ(0u, stats->mnv_context_accepted);
        ASSERT_EQ(0u, stats->mnv_context_fallback_mismatch);
        ASSERT_EQ(0u, stats->mnv_context_fallback_unsupported);
        ASSERT_EQ(0u, stats->del_context_accepted);
        ASSERT_EQ(0u, stats->del_context_fallback_mismatch);
        ASSERT_EQ(0u, stats->del_context_fallback_unsupported);
        ASSERT_EQ(0u, stats->ins_context_accepted);
        ASSERT_EQ(0u, stats->ins_context_fallback_mismatch);
        ASSERT_EQ(0u, stats->ins_context_fallback_unsupported);
        ASSERT_EQ(1u, stats->indel_context_accepted);
        ASSERT_EQ(0u, stats->indel_context_fallback_unsupported);
        duckvep_workspace_close(ws);
        duckvep_options_close(opts);
        duckvep_model_close(model);
    }
...
TEST sequence_delta_with_scratch_indel_known_scene(void) {
    ...
    for (case_idx = 0u; case_idx < 2u; case_idx++) {
         ...
         * CodingContext now resolves the frameshift the legacy body-only path skipped. */
        ASSERT(kprop_delta_is_frameshift_at(&delta, 3));
        duckvep_sequence_delta_fill_with_scratch(DUCKVEP_KIND_INDEL, &s.tx, &s.ex,
                                                 &s.seq, &s.v, 0u, 0u, s.vpos,
                                                 s.strand, NULL, &delta);
...
TEST annotate_inframe_delins_boundary_no_route_known_scene(void) {
    static uint8_t cds[18] = {
        'A','T','G',  'A','A','A',  'C','C','C',  'G','G','G',  'T','T','T',  'G','A','A'
    };
    static const uint8_t alt3[3] = { 'G','C','C' };
...
TEST annotate_inframe_insertion_route_known_scene(void) {
    ...
    for (case_idx = 0u; case_idx < 2u; case_idx++) {
        ...
        ASSERT_EQ(0u, stats->del_context_fallback_mismatch);
        ASSERT_EQ(0u, stats->del_context_fallback_unsupported);
        ASSERT_EQ(1u, stats->ins_context_accepted);
        ASSERT_EQ(0u, stats->ins_context_fallback_mismatch);
        ASSERT_EQ(0u, stats->ins_context_fallback_unsupported);
...

    {
        ...
        ASSERT_EQ(0u, stats->ins_context_accepted);
        ASSERT_EQ(0u, stats->ins_context_fallback_mismatch);
        ASSERT_EQ(1u, stats->ins_context_fallback_unsupported);
...
TEST annotate_inframe_deletion_route_known_scene(void) {
    ...
    for (case_idx = 0u; case_idx < 2u; case_idx++) {
        ...
        ASSERT_EQ(0u, stats->mnv_context_fallback_mismatch);
        ASSERT_EQ(0u, stats->mnv_context_fallback_unsupported);
        ASSERT_EQ(1u, stats->del_context_accepted);
        ASSERT_EQ(0u, stats->del_context_fallback_mismatch);
        ASSERT_EQ(0u, stats->del_context_fallback_unsupported);
...
TEST sequence_delta_annotation_wrapper_del_insufficient_scratch_known(void) {
    ...
    uint8_t stack_alt_pep[16];
    duckvep_delta_scratch_t scratch;
    duckvep_sequence_delta_t legacy;
    ...
    duckvep_sequence_delta_fill(DUCKVEP_KIND_DEL, &s.tx, &s.ex, &s.seq,
                                &s.v, 0u, 0u, s.vpos, s.strand, &legacy);
    duckvep_sequence_delta_fill_for_annotation_trace(DUCKVEP_KIND_DEL, &s.tx, &s.ex,
                                                     &s.seq, &s.v, 0u, 0u, s.vpos,
                                                     s.strand, &scratch, &route,
                                                     &routed);
    ASSERT(kprop_delta_is_inframe_deletion_at(&legacy, 2));
    ASSERT(kprop_sequence_delta_equal(&legacy, &routed));
    ASSERT_EQ(DUCKVEP_DELTA_ROUTE_DEL_CONTEXT_FALLBACK_UNSUPPORTED, route);
    PASS();
}
...
TEST sequence_delta_annotation_wrapper_start_lost_mnv(void) {
    ...
    duckvep_sequence_delta_fill(DUCKVEP_KIND_MNV, &s.tx, &s.ex, &s.seq, &s.v,
                                0u, 0u, s.vpos, s.strand, &legacy);
    duckvep_sequence_delta_fill_with_scratch(DUCKVEP_KIND_MNV, &s.tx, &s.ex, &s.seq,
                                             &s.v, 0u, 0u, s.vpos, s.strand,
                                             &scratch, &direct);
    duckvep_sequence_delta_fill_for_annotation(DUCKVEP_KIND_MNV, &s.tx, &s.ex,
                                               &s.seq, &s.v, 0u, 0u, s.vpos,
                                               s.strand, &scratch, &routed);
    ASSERT(legacy.valid);
    ASSERT(direct.valid);
    ASSERT(routed.valid);
    ASSERT(kprop_sequence_delta_equal(&legacy, &direct));
    ASSERT(kprop_sequence_delta_equal(&legacy, &routed));
...
static int kprop_cross_codon_scene_deltas(
    ...
    if (cds == NULL || alt_tx == NULL || direct == NULL || legacy == NULL || routed == NULL ||
...
TEST sequence_delta_with_scratch_cross_codon_known_scene(void) {
    ...
    ASSERT(kprop_cross_codon_scene_deltas(1, cds, 15u, 6u, 2u, alt_tx, 2u,
                                          &direct, &legacy, &routed, &route));
    ASSERT(kprop_delta_is_coarse_cross_codon_missense(&direct));
    ASSERT(kprop_delta_is_coarse_cross_codon_missense(&legacy));
    ASSERT(kprop_sequence_delta_equal(&legacy, &direct));
    ASSERT(kprop_sequence_delta_equal(&legacy, &routed));
    ASSERT_EQ(DUCKVEP_DELTA_ROUTE_MNV_CONTEXT_ACCEPTED, route);
    PASS();
}
...
TEST sequence_delta_with_scratch_cross_codon_negative_scenes(void) {
     ...
     * and >3-base-wide windows). The legacy per-shape filler still only knows the missense
    ...
    ASSERT(direct.valid && direct.synonymous && direct.protein_pos == -1 &&
           ...
           !direct.stop_retained && !direct.start_lost);
    ASSERT(!legacy.valid);
    ...
    ASSERT(!legacy.valid);
    ASSERT(!routed.valid);
    ASSERT_EQ(DUCKVEP_DELTA_ROUTE_MNV_CONTEXT_FALLBACK_UNSUPPORTED, route);
    PASS();
}
...
static enum theft_trial_res prop_delta_annotation_wrapper_matches_legacy(struct theft *t,
                                                                         void *arg1) {
    const struct kprop_coding *s = (const struct kprop_coding *)arg1;
    duckvep_haplotype_edit_t edits[4];
    uint8_t alt_cds[64];
    ...
    duckvep_sequence_delta_fill_for_annotation((duckvep_variant_kind_t)s->vkind,
                                               ...
                                               &scratch, &routed);
    if (!legacy.valid || !routed.valid) return THEFT_TRIAL_FAIL;
    if (!kprop_sequence_delta_equal(&legacy, &routed)) return THEFT_TRIAL_FAIL;
...
TEST sequence_delta_annotation_wrapper_matches_legacy(void) {
    struct theft_run_config cfg;
    memset(&cfg, 0, sizeof cfg);
    cfg.name = "sequence delta annotation wrapper MNV == legacy";
    cfg.prop1 = prop_delta_annotation_wrapper_matches_legacy;
    cfg.type_info[0] = &kprop_context_delta_info;
    cfg.trials = kprop_env_u64("DUCKVEP_PROP_TRIALS", KPROP_DEFAULT_TRIALS);
    cfg.seed = (theft_seed)kprop_env_u64("DUCKVEP_PROP_SEED", KPROP_DEFAULT_SEED);
...
static enum theft_trial_res prop_cursor_mnv_route_matches_tile(struct theft *t,
                                                               void *arg1) {
    const struct kprop_coding *s = (const struct kprop_coding *)arg1;
    duckvep_model_t *model = NULL;
    duckvep_options_t *opts = NULL;
    ...
    if (tile_stats.mnv_context_accepted == 0u ||
        tile_stats.mnv_context_fallback_mismatch != 0u ||
        tile_stats.mnv_context_fallback_unsupported != 0u ||
        tile_stats.mnv_context_accepted != cursor_stats.mnv_context_accepted ||
        tile_stats.mnv_context_fallback_mismatch != cursor_stats.mnv_context_fallback_mismatch ||
        tile_stats.mnv_context_fallback_unsupported != cursor_stats.mnv_context_fallback_unsupported) {
        res = THEFT_TRIAL_FAIL; goto done;
    }
    {
        size_t i;
...
TEST annotate_cursor_mnv_route_matches_tile_for_any_output_split(void) {
    struct theft_run_config cfg;
    memset(&cfg, 0, sizeof cfg);
    cfg.name = "annotate cursor MNV route == tile under output splits";
    cfg.prop1 = prop_cursor_mnv_route_matches_tile;
    cfg.type_info[0] = &kprop_context_delta_info;
    cfg.trials = kprop_env_u64("DUCKVEP_PROP_TRIALS", KPROP_DEFAULT_TRIALS);
    cfg.seed = (theft_seed)kprop_env_u64("DUCKVEP_PROP_SEED", KPROP_DEFAULT_SEED);
...
static struct { uint32_t accepted; uint32_t unsupported; uint32_t fwd; uint32_t rev; uint32_t len2; uint32_t len3; } g_cursor_cross_route_cov;
...
static enum theft_trial_res prop_cursor_cross_codon_mnv_route_matches_tile(
    ...
    if (tile_stats.mnv_context_accepted != cursor_stats.mnv_context_accepted ||
        tile_stats.mnv_context_fallback_mismatch != cursor_stats.mnv_context_fallback_mismatch ||
        tile_stats.mnv_context_fallback_unsupported != cursor_stats.mnv_context_fallback_unsupported ||
        tile_stats.mnv_context_fallback_mismatch != 0u) {
        res = THEFT_TRIAL_FAIL; goto done;
    ...
    /* Every generator mode (missense / synonymous / stop-gained cross-codon window) now
     * resolves through the authoritative interpreter, so the route is always ACCEPTED. The
    ...
    if (tile_stats.mnv_context_accepted != 1u ||
        tile_stats.mnv_context_fallback_unsupported != 0u) {
        res = THEFT_TRIAL_FAIL; goto done;
    }
    g_cursor_cross_route_cov.accepted++;
...
TEST annotate_cursor_cross_codon_mnv_route_matches_tile_for_any_output_split(void) {
    ...
    ASSERT_EQ(THEFT_RUN_PASS, theft_run(&cfg));
    ASSERT(g_cursor_cross_route_cov.accepted > 0u);
    ...
    fprintf(stderr,
            "[cursor-cross-route coverage] accepted=%u unsupported=%u fwd=%u rev=%u len2=%u len3=%u\n",
            g_cursor_cross_route_cov.accepted, g_cursor_cross_route_cov.unsupported,
...
static enum theft_trial_res prop_delta_scratch_mnv_matches_codon_oracle(struct theft *t,
    ...
    if (delta.start_lost || delta.frameshift || delta.inframe_deletion ||
...
static enum theft_trial_res prop_delta_scratch_cross_codon_mnv_matches_oracle(
    ...
    if (route != DUCKVEP_DELTA_ROUTE_MNV_CONTEXT_ACCEPTED) return THEFT_TRIAL_FAIL;
    ...
    if (direct.synonymous != (uint8_t)f.synonymous ||
        ...
        direct.start_lost || direct.frameshift || direct.inframe_deletion ||
        direct.inframe_insertion || direct.protein_altering) {
        return THEFT_TRIAL_FAIL;
    }
    /* The legacy per-shape filler only ever resolved the missense cross-codon window; the
     * synonymous and stop-gained windows are exactly what the authoritative interpreter adds. */
    if (f.missense) {
        if (!legacy.valid || !kprop_delta_is_coarse_cross_codon_missense(&legacy)) {
            return THEFT_TRIAL_FAIL;
        }
        g_delta_cross_scratch_cov.missense++;
    } else if (f.synonymous) {
        if (legacy.valid) return THEFT_TRIAL_FAIL;
...
TEST annotate_frameshift_indel_matches_oracle(void) {
    ...
    fprintf(stderr,
            "[frameshift coverage] ins=%u del=%u delins=%u(+1=%u +2=%u -1=%u -2=%u) reverse=%u stop_gained=%u\n",
...
static enum theft_trial_res prop_annotate_inframe_deletion_matches_oracle(struct theft *t, void *arg1) {
    ...
    stats = duckvep_workspace_delta_route_stats(ws);
    if (stats == NULL || stats->del_context_accepted != 1u ||
        stats->del_context_fallback_mismatch != 0u ||
        stats->del_context_fallback_unsupported != 0u ||
        stats->mnv_context_accepted != 0u ||
        stats->mnv_context_fallback_mismatch != 0u ||
        stats->mnv_context_fallback_unsupported != 0u) {
        tr = THEFT_TRIAL_FAIL; goto done;
    }
...
static enum theft_trial_res prop_cursor_del_route_matches_tile(struct theft *t, void *arg1) {
    ...
    if (tile_stats.del_context_accepted != 1u ||
        tile_stats.del_context_fallback_mismatch != 0u ||
        tile_stats.del_context_fallback_unsupported != 0u ||
        tile_stats.mnv_context_accepted != 0u ||
        tile_stats.mnv_context_fallback_mismatch != 0u ||
        tile_stats.mnv_context_fallback_unsupported != 0u ||
        tile_stats.del_context_accepted != cursor_stats.del_context_accepted ||
        tile_stats.del_context_fallback_mismatch != cursor_stats.del_context_fallback_mismatch ||
        tile_stats.del_context_fallback_unsupported != cursor_stats.del_context_fallback_unsupported) {
        res = THEFT_TRIAL_FAIL; goto done;
    }
...
static struct { uint32_t fwd; uint32_t rev; uint32_t lengthen; uint32_t shorten; }
    g_context_inframe_delins_cov;

static enum theft_trial_res prop_context_inframe_delins_matches_oracle(struct theft *t,
                                                                      void *arg1) {
    const struct kprop_coding *s = (const struct kprop_coding *)arg1;
    duckvep_haplotype_edit_t edits[4];
    uint8_t alt_cds[96];
    ...
    if (!ctx.has_single_edit || ctx.applied_edits != 1u ||
        ...
        (ctx.single_edit_ref_len % 3u) != 0u ||
        (ctx.single_edit_alt_len % 3u) != 0u || ctx.length_diff == 0 ||
        (ctx.length_diff % 3) != 0 || ctx.single_edit_cds_start <= 3u ||
        ((ctx.single_edit_cds_start - 1u) % 3u) != 0u) {
        return THEFT_TRIAL_FAIL;
    }
    ...
    if (ctx.length_diff > 0) {
        if (!kprop_delta_is_inframe_insertion_at(&delta, protein_pos)) return THEFT_TRIAL_FAIL;
        g_context_inframe_delins_cov.lengthen++;
    } else {
        if (!kprop_delta_is_inframe_deletion_at(&delta, protein_pos)) return THEFT_TRIAL_FAIL;
        g_context_inframe_delins_cov.shorten++;
    }
    if (s->strand > 0) g_context_inframe_delins_cov.fwd++;
    else g_context_inframe_delins_cov.rev++;
    return THEFT_TRIAL_PASS;
}

TEST coding_context_delta_inframe_delins_matches_oracle(void) {
    struct theft_run_config cfg;
    memset(&cfg, 0, sizeof cfg);
    cfg.name = "coding context delta in-frame delins == edit-origin oracle";
    cfg.prop1 = prop_context_inframe_delins_matches_oracle;
    cfg.type_info[0] = &kprop_inframe_delins_info;
    cfg.trials = kprop_env_u64("DUCKVEP_PROP_TRIALS", KPROP_DEFAULT_TRIALS);
    cfg.seed = (theft_seed)kprop_env_u64("DUCKVEP_PROP_SEED", KPROP_DEFAULT_SEED);
    memset(&g_context_inframe_delins_cov, 0, sizeof g_context_inframe_delins_cov);
    ASSERT_EQ(THEFT_RUN_PASS, theft_run(&cfg));
    ASSERT(g_context_inframe_delins_cov.fwd > 0u);
    ASSERT(g_context_inframe_delins_cov.rev > 0u);
    ASSERT(g_context_inframe_delins_cov.lengthen > 0u);
    ASSERT(g_context_inframe_delins_cov.shorten > 0u);
    fprintf(stderr,
            "[context-inframe-delins coverage] forward=%u reverse=%u lengthen=%u shorten=%u\n",
            g_context_inframe_delins_cov.fwd, g_context_inframe_delins_cov.rev,
            g_context_inframe_delins_cov.lengthen, g_context_inframe_delins_cov.shorten);
    PASS();
}
...
static enum theft_trial_res prop_delta_scratch_indel_matches_oracle(struct theft *t,
    ...
    if (ctx.length_diff > 0) {
        if (!kprop_delta_is_inframe_insertion_at(&delta, protein_pos)) return THEFT_TRIAL_FAIL;
        g_delta_scratch_indel_cov.lengthen++;
    } else if (ctx.length_diff < 0) {
        if (!kprop_delta_is_inframe_deletion_at(&delta, protein_pos)) return THEFT_TRIAL_FAIL;
        g_delta_scratch_indel_cov.shorten++;
    } else return THEFT_TRIAL_FAIL;
...
TEST sequence_delta_with_scratch_indel_matches_oracle(void) {
    struct theft_run_config cfg;
    memset(&cfg, 0, sizeof cfg);
    cfg.name = "sequence delta scratch INDEL == in-frame delins oracle";
...
static enum theft_trial_res prop_annotate_inframe_insertion_matches_oracle(struct theft *t, void *arg1) {
    ...
    stats = duckvep_workspace_delta_route_stats(ws);
    if (stats == NULL || stats->mnv_context_accepted != 0u ||
        stats->mnv_context_fallback_mismatch != 0u ||
        stats->mnv_context_fallback_unsupported != 0u ||
        stats->del_context_accepted != 0u ||
        stats->del_context_fallback_mismatch != 0u ||
        stats->del_context_fallback_unsupported != 0u ||
        stats->ins_context_accepted != 1u ||
        stats->ins_context_fallback_mismatch != 0u ||
        stats->ins_context_fallback_unsupported != 0u) {
        tr = THEFT_TRIAL_FAIL; goto done;
    }
...
static enum theft_trial_res prop_cursor_ins_route_matches_tile(struct theft *t, void *arg1) {
    ...
    if (!saw_full || tile_n != cursor_n || tile_n != 1u) { res = THEFT_TRIAL_FAIL; goto done; }
    if (tile_stats.ins_context_accepted != 1u ||
        tile_stats.ins_context_fallback_mismatch != 0u ||
        tile_stats.ins_context_fallback_unsupported != 0u ||
        tile_stats.mnv_context_accepted != 0u ||
        tile_stats.mnv_context_fallback_mismatch != 0u ||
        tile_stats.mnv_context_fallback_unsupported != 0u ||
        tile_stats.del_context_accepted != 0u ||
        tile_stats.del_context_fallback_mismatch != 0u ||
        tile_stats.del_context_fallback_unsupported != 0u ||
        tile_stats.ins_context_accepted != cursor_stats.ins_context_accepted ||
        tile_stats.ins_context_fallback_mismatch != cursor_stats.ins_context_fallback_mismatch ||
        tile_stats.ins_context_fallback_unsupported != cursor_stats.ins_context_fallback_unsupported) {
        res = THEFT_TRIAL_FAIL; goto done;
    }
    if (!consequence_rows_equal(&tile_rows[0], &cursor_rows[0])) { res = THEFT_TRIAL_FAIL; goto done; }
    anchor_cds = kprop_cds_pos_for_genomic(s, s->vpos);
...
static enum theft_alloc_res tileprop_alloc(struct theft *t, void *env, void **instance) {
    ...
    for (i = 0; i < s->n; i++) {
        ...
        s->chrom[i] = run;
        s->pos[i] = pos;
        s->ref[i] = bases[theft_random_bits(t, 2)];
        s->alt[i] = bases[theft_random_bits(t, 2)];
    }
...
int main(int argc, char **argv) {
    ...
    RUN_TEST(sweep_allele_normalized_candidates_match_trim_oracle);
    ...
    RUN_TEST(coding_context_delta_inframe_delins_known_scene);
    ...
    RUN_TEST(annotate_inframe_delins_boundary_no_route_known_scene);
    ...
    RUN_TEST(sequence_delta_annotation_wrapper_matches_legacy);
    RUN_TEST(annotate_cursor_mnv_route_matches_tile_for_any_output_split);
    ...
    RUN_TEST(coding_context_delta_inframe_delins_matches_oracle);
    ...
    RUN_TEST(annotate_rejects_prefix_zero_pure_insertion);
