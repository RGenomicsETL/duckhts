-- Regenerate the offline DuckVEP Ensembl fixtures from the exact public
-- release-116 core and funcgen databases. Run from the repository root after
...
--   ./test/scripts/prepare_test_data.sh --duckvep-only
--
...
ATTACH
  'host=ensembldb.ensembl.org port=5306 user=anonymous database=homo_sapiens_funcgen_116_38'
  AS ensembl_funcgen_grch38 (TYPE mysql, READ_ONLY);
ATTACH
...
CREATE OR REPLACE MACRO fixture_regulatory_features(
  source_catalog, region_ids_sql
) AS TABLE
FROM mysql_query(source_catalog,
  'SELECT * FROM regulatory_feature '
  'WHERE seq_region_id IN (' || region_ids_sql || ')'
);

CREATE OR REPLACE MACRO fixture_regulatory_feature_types(
  source_catalog, region_ids_sql
) AS TABLE
FROM mysql_query(source_catalog,
  'SELECT ft.* FROM feature_type ft '
  'WHERE EXISTS (SELECT 1 FROM regulatory_feature rf '
  'WHERE rf.seq_region_id IN (' || region_ids_sql || ') '
  'AND rf.feature_type_id = ft.feature_type_id)'
);

CREATE OR REPLACE MACRO fixture_motif_features(
  source_catalog, region_ids_sql
) AS TABLE
FROM mysql_query(source_catalog,
  'SELECT * FROM motif_feature '
  'WHERE seq_region_id IN (' || region_ids_sql || ')'
);

CREATE OR REPLACE MACRO fixture_reference_chunks(
  source_catalog, assembly_name, region_ids_sql, expected_region_count
) AS TABLE
WITH components AS MATERIALIZED (
  ...
  SELECT CASE
    WHEN (SELECT count(DISTINCT seq_region_id) FROM ordered) !=
         expected_region_count THEN
...
FROM fixture_coord_systems(
  'ensembl_grch38', 'GRCh38', '131066,132907,2006446537'
);
...
FROM fixture_reference_chunks(
  'ensembl_grch38', 'GRCh38', '131066,132907,2006446537', 3
);

CREATE OR REPLACE SCHEMA duckvep_grch38_funcgen;
CREATE TABLE duckvep_grch38_funcgen.regulatory_feature AS
FROM fixture_regulatory_features(
  'ensembl_funcgen_grch38', '131066,132907,2006446537'
);
CREATE TABLE duckvep_grch38_funcgen.feature_type AS
FROM fixture_regulatory_feature_types(
  'ensembl_funcgen_grch38', '131066,132907,2006446537'
);
CREATE TABLE duckvep_grch38_funcgen.motif_feature AS
FROM fixture_motif_features(
  'ensembl_funcgen_grch38', '131066,132907,2006446537'
);
...
FROM fixture_reference_chunks(
  'ensembl_grch37', 'GRCh37', '100965601,27742', 2
);
...
INSERT INTO expected_reference_hashes VALUES
  ...
  ('GRCh38', 'KI270395.1', '2cdc20e2bacfba28af4848078c9e0c5b0380d0e3d6954281a280c148ca482f3d'),
...
CREATE TEMP TABLE canonical_reference_manifests AS
SELECT assembly,
       sha256(string_agg(sequence_sha256, '' ORDER BY chrom)) AS reference_sha256
FROM actual_reference_hashes
GROUP BY assembly;

CREATE TEMP TABLE source_manifests AS
SELECT 'GRCh38'::VARCHAR AS assembly,
       'homo_sapiens_core_116_38'::VARCHAR AS source_database,
       sha256(content) AS source_manifest_sha256
FROM read_text(
  'https://ftp.ensembl.org/pub/release-116/mysql/homo_sapiens_core_116_38/CHECKSUMS'
)
UNION ALL
SELECT 'GRCh38'::VARCHAR AS assembly,
       'homo_sapiens_funcgen_116_38'::VARCHAR AS source_database,
       sha256(content) AS source_manifest_sha256
FROM read_text(
  'https://ftp.ensembl.org/pub/release-116/mysql/homo_sapiens_funcgen_116_38/CHECKSUMS'
)
UNION ALL
SELECT 'GRCh37'::VARCHAR AS assembly,
       'homo_sapiens_core_116_37'::VARCHAR AS source_database,
       sha256(content) AS source_manifest_sha256
FROM read_text(
...
SELECT CASE
  WHEN count(*) != 3 OR
       max(source_manifest_sha256) FILTER (
         WHERE source_database = 'homo_sapiens_core_116_38'
       ) != 'c5afd2ec87ad7aa402035a070e81068e7bd8f4c5bdae9f3c698f8ff5b2fd63d1' OR
       max(source_manifest_sha256) FILTER (
         WHERE source_database = 'homo_sapiens_funcgen_116_38'
       ) != '600d0ce6d6bbad3da4c4f2f3eede0311e1fb6497ceb5e6785612b7ab7cf778c8' OR
       max(source_manifest_sha256) FILTER (
         WHERE source_database = 'homo_sapiens_core_116_37'
...
CREATE TEMP TABLE canonical_source_manifests AS
SELECT assembly,
       sha256(string_agg(
         source_database || '\t' || source_manifest_sha256 || '\n',
         '' ORDER BY source_database
       )) AS source_manifest_sha256,
       string_agg(source_database, ',' ORDER BY source_database) AS source_databases
FROM source_manifests
GROUP BY assembly;
...
SELECT CASE
  WHEN count(*) != 2
    OR max(model_sha256) FILTER (WHERE assembly = 'GRCh38') !=
       'd5e9d7efe42f561ffe3a7ab9e7871bdd510c603871ac3699874cabc46b82f9db'
    OR max(model_sha256) FILTER (WHERE assembly = 'GRCh37') !=
       '0a11b81b8c9ac0c0550e153cc8903650c91b305e826c4d844394c2df2f92b007'
    OR max(region_count) FILTER (WHERE assembly = 'GRCh38') != 3
    OR max(reference_base_count) FILTER (WHERE assembly = 'GRCh38') != 82401
    ...
    OR max(regulation_feature_count) FILTER (WHERE assembly = 'GRCh38') != 3
    OR max(regulatory_region_count) FILTER (WHERE assembly = 'GRCh38') != 0
    OR max(motif_feature_count) FILTER (WHERE assembly = 'GRCh38') != 3
    OR max(region_count) FILTER (WHERE assembly = 'GRCh37') != 2
    ...
    OR max(regulation_feature_count) FILTER (WHERE assembly = 'GRCh37') != 0
    OR max(regulatory_region_count) FILTER (WHERE assembly = 'GRCh37') != 0
    OR max(motif_feature_count) FILTER (WHERE assembly = 'GRCh37') != 0 THEN
...
SELECT
  r.source_name,
  r.source_version,
  s.source_databases AS source_database,
  r.assembly,
  CASE r.assembly
    WHEN 'GRCh38' THEN 'MT,KI270395.1,HG2047_PATCH'
    ELSE 'MT,GL000201.1'
  ...
  r.regulation_feature_count,
  r.regulatory_region_count,
  r.motif_feature_count,
  r.transcript_filter
FROM fixture_receipts r
JOIN canonical_source_manifests s USING (assembly)
...
COPY duckvep_grch38_core.translation_attrib TO
  ...
  (FORMAT PARQUET, COMPRESSION ZSTD);
COPY duckvep_grch38_funcgen.feature_type TO
  'test/data/duckvep/ensembl_core/grch38/funcgen_feature_type.parquet'
  (FORMAT PARQUET, COMPRESSION ZSTD);
COPY duckvep_grch38_funcgen.regulatory_feature TO
  'test/data/duckvep/ensembl_core/grch38/funcgen_regulatory_feature.parquet'
  (FORMAT PARQUET, COMPRESSION ZSTD);
COPY duckvep_grch38_funcgen.motif_feature TO
  'test/data/duckvep/ensembl_core/grch38/funcgen_motif_feature.parquet'
  (FORMAT PARQUET, COMPRESSION ZSTD);
