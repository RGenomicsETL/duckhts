...
`r if (params$comparison == "reserve") "This focused comparison measures the aligned-block reservation change against origin/develop in separate R processes on the same host and inputs." else "This focused comparison measures the old CIGAR implementation and the shared checked decoder in separate R processes on the same host and inputs."` It measures
...
`r if (params$comparison == "reserve") "On the measured host, uptime load averages (1/5/15 minutes) were 1.40/1.77/1.23 before baseline, 2.27/2.01/1.35 before candidate, and 1.47/1.81/1.32 afterward. The 8,192-row synthetic text aligned-block workload remains 0.020 to 0.022 seconds (median, +10%). A separate 300-execution perf record -F 199 -g -e cycles:u sample of that materialized text input and the same hash summary found 5.58% of candidate cycles in libc __memmove_avx_unaligned_erms (5.32% via duckdb::VectorListBuffer::Reserve); baseline __memmove was below 1%. Child-list copies on geometric growth are consistent with the residual synthetic cost; the sample does not isolate each reserve call's latency." else ""`
...
The commands below use the CSV prefix `r paste0("benchmarks/", params$prefix)`.
...
taskset -c "$CPU" Rscript scripts/benchmark_cigar_validation.R baseline \
  ...
  clean `r paste0("benchmarks/", params$prefix)` full 5
taskset -c "$CPU" Rscript scripts/benchmark_cigar_validation.R candidate \
  ...
  "$CANDIDATE_SOURCE_STATE" `r paste0("benchmarks/", params$prefix)` full 5
Rscript -e 'rmarkdown::render("benchmarks/benchmark_cigar_validation.Rmd",
  params = list(prefix = "`r params$prefix`", comparison = "`r params$comparison`"),
  output_file = "benchmark_cigar_`r if (params$comparison == "reserve") "reserve" else "validation"`.md")'
```
