...
- add `no_left_align` parameter to
  ...
  (default `FALSE`): when `TRUE`, skips post-liftover left-alignment of
  indels, mirroring `bcftools +liftover --no-left-align`

- harden
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md)
  filter-expression failures for installed-package CI runs on
  Linux/Windows: invalid `include`/`exclude` expressions now surface as
  normal package errors from the bundled score engine instead of relying
  on a cross-frame bcftools shim jump path

- extend
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md)
  with bcftools-style filtering arguments (`regions`, `regions_file`,
  `regions_overlap`, `targets`, `targets_file`, `targets_overlap`,
  `apply_filters`, `include`, `exclude`) and package-level tests for
  region/target/FILTER behavior; `include`/`exclude` expressions are now
  evaluated against core VCF fields (`POS`, `QUAL`, `CHROM`, `ID`,
  `REF`, `ALT`, `FILTER`)

- improve
  [`rduckhts_liftover()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_liftover.md)
  multiallelic semantic parity with upstream `bcftools +liftover`:
  preserve all ALT alleles through reference-introduction/swaps, apply
  dynamic-allele indel normalization/left-alignment, treat `ALT='.'` as
  no alternate alleles, and add tinytest coverage for forward/reverse
  multiallelic liftover outputs

- add GWAS-VCF multi-PRS scoring:
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md)
  now supports GWAS-VCF summary files where each sample in the VCF
  defines a separate PRS with `FORMAT/ES` and `FORMAT/LP` fields

- add INFO/END liftover:
  [`rduckhts_liftover()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_liftover.md)
  gains an `end_pos_col` parameter to lift INFO/END positions alongside
  the primary coordinate, producing a `dest_end` output column

- add mitochondria passthrough:
  [`rduckhts_liftover()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_liftover.md)
  gains a `lift_mt` parameter (default `FALSE`) that renames MT/chrM
  contigs with matching source/destination sizes without chain lifting,
  matching upstream `bcftools +liftover`

- harden
  [`rduckhts_liftover()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_liftover.md)
  wrapper validation: negative `max_snp_gap`/`max_indel_inc` now fail in
  R before SQL execution to keep tinytest/check-r-package behavior
  stable across CI platforms

- add fai-only mode:
  [`rduckhts_munge()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_munge.md)
  `fasta_ref` parameter is now optional (default `NULL`); when omitted,
  alleles pass through as-is without reference matching, matching
  upstream `bcftools +munge --fai`-only behavior

- port full indel liftover pipeline from upstream `bcftools +liftover`
  (Tier 3 — Algorithmic Completeness): allele extension,
  Needleman-Wunsch re-alignment with BWA-MEM affine gap scoring, 3-phase
  `find_reference` with interior matching and SNP rescue, left-alignment
  post-liftover via VT normalization, and SNP retry-as-indel dispatch —
  [`rduckhts_liftover()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_liftover.md)
  now handles indels near chain gaps, difficult SNPs at chain
  boundaries, and padded variants with correct coordinate adjustment

- **breaking**: some previously-rejected edge-of-chain SNPs now map
  successfully with `note = 'Padded'` due to the upstream-compatible
  retry-as-indel dispatch; R test expectations updated accordingly

- harden
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md)
  preset column parity: add 45 missing column entries across all 8
  presets so non-scoring columns (A2, FRQ, SE, N, INFO, etc.) are
  recognized and silently consumed during header matching, matching
  upstream `score.h` definitions

- add gzipped summary file support to
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md):
  `.gz`/`.bgz` compressed PGS Catalog and GWAS summary files can now be
  passed directly without decompression

- fix
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md)
  PRS name derivation to strip multiple known extensions
  (`{gz,txt,tsv,vcf,bcf}`) iteratively, matching upstream behavior

- add validation for explicitly requested `use` tag in
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md):
  error if the VCF header lacks the specified FORMAT field instead of
  silently scoring zero

- fix
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md)
  CNT column naming to match upstream: when `q_score_thr` and `counts`
  are both active, count columns are now named `<prs>_CNT_p<thr>`
  (upstream pattern) instead of `<prs>_p<thr>_CNT` (**breaking** column
  name change for q_score_thr + counts queries)

- fix
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md)
  threshold boundary precision to match upstream: parse `q_score_thr`
  values with `strtof`→double promotion before `-log10`, reproducing the
  exact float→double comparison asymmetry in upstream `bcftools +score`;
  markers at exact P-value boundaries may now be excluded where they
  were previously included

- fix
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md)
  wrong-result bugs: remove incorrect METAL `Zscore → P` and SSF
  `standard_error → P` column mappings, add haploid GT support for chrX
  male samples, and fix memory leak on skipped markers

- fix
  [`rduckhts_liftover()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_liftover.md)
  wrong-result bugs: skip reverse complement for symbolic alleles and
  detect insertions as indels in the liftover path

- fix
  [`rduckhts_munge()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_munge.md)
  wrong-result bugs: emit `filter = 'MissingContig'` instead of aborting
  when FASTA ref fetch fails for unknown contigs, and propagate NAN
  correctly on AC swap with missing NS

- harden
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md)
  upstream parity: port flexible chromosome name resolution (chr prefix
  strip/prepend, 23→X, 24→Y, 26/MT→chrM aliases), numerically stable
  `-log10(p)` parsing for very small p-values, and NA/`.` missing value
  handling in summary stats fields (BETA, OR, P, LP)

- add comprehensive
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md)
  test coverage: DS/HDS/AP/GP dosage modes with real FORMAT values,
  OR-to-beta conversion, PLINK2 preset with LOG10_P, custom columns_file
  mapping, allele mismatch (zero-match), missing genotype (`./.`)
  handling, NA in summary stats, chr-prefix flexible matching,
  auto-detection priority, and small p-value threshold precision

- strengthen
  [`rduckhts_score()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_score.md)
  parity coverage for variant matching by adding rsID-based
  fixture/tests (`use_variant_id = TRUE`), explicit CHR/BP mismatch
  behavior checks, and an error assertion when marker IDs are required
  but missing

- add METAL meta-analysis output support:
  [`rduckhts_munge()`](https://rgenomicsetl.github.io/duckhts/reference/rduckhts_munge.md)
  auto-detects METAL/heterogeneity keys (`INFO`, `HET_I2`, `HET_P`,
  `HET_LP`, `DIRE`) in the resolved column map and dispatches to
  `duckdb_munge_metal(...)`, returning `si`, `i2`, `cq`, and `ed`
  columns; non-METAL presets continue to use the base 16-column schema
  without these fields

- align bundled munge outputs with the extension-level schema by
  renaming `swapped` to `alleles_swapped` with explicit
  allele-orientation semantics
...
- harden bundled liftover upstream parity: full IUPAC complement table,
  IUPAC→N reference sequence sanitization, `25→X`/`26→MT` contig
  aliases, Ensembl chain ID dedup, chain coverage validation, and
  `uint64_t` regidx payload — all exact ports from upstream
  `bcftools +liftover`
