...
stopifnot(
  nrow(human_evidence) == 9L,
  length(unique(human_evidence$source_revision)) == 1L,
...
Keep the thread count fixed for the complete run. Execute the
...
Measured transcript/core-feature sweep, consequence classification, NMD and
HGVS rendering: `r human_measure("DuckVEP_rich_HGVS_regulatory", "annotation rows")`.
...
RegionKey bounds provide a cheap ordered rejection before exact half-open
coordinate comparison. The chromosome equality is still explicit: encoded
bounds accelerate the plan but do not define biological overlap. DuckDB may
execute the two range inequalities with
...
`r human_measure("Ensembl_regulatory_IEJoin", "matched alleles")`.
...
lets DuckDB project or omit them normally. The final `ORDER BY` makes the
Parquet row order deterministic; ZSTD compression and file writing are included
in this stage's measurement.
...
Measured late joins and ZSTD Parquet materialization:
`r human_measure("compose_gene_constraint_and_write_ZSTD_Parquet", "annotation rows written")`.
...
chromosomes 1--22, X, Y, and MT and materialized
`r format(human_evidence$result_rows[human_evidence$stage == "DuckVEP_rich_HGVS_regulatory"], big.mark = ",")`
consequence/core-feature rows. It was deliberately run as one warm-page-cache,
unpinned integration pass and completed in
`r human_measure("whole_process_warm_page_cache", "annotation rows written")`.
Peak process RSS was
`r sprintf("%.2f GiB", human_evidence$max_rss_kb[human_evidence$stage == "whole_process_warm_page_cache"] / 1024^2)`
because this pass retained the allele, interval-match, and complete consequence
relations before the ordered write. It validates composition and exposes the
current materialization cost rather than replacing the pinned engine tables below.
