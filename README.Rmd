...
This bundled deterministic model demonstrates the load, rich/HGVS annotation,
SO decoding, and release lifecycle without downloading an Ensembl release.
...
For high-cardinality pipelines, filter compact integer masks/codes first and
decode selected SO bits with `duckvep_so_terms()` afterward. Join
`transcript_index`, `gene_index`, and `regulation_feature_index` to the prepared
model relations for stable IDs and cold metadata. Exact/positional resources such
as ClinVar or population frequencies, interval resources such as constraint or
regulatory tracks, and gene/disease resources stay as typed Parquet/DuckLake
relations; DuckDB can apply exact joins, range prefilters, IEJoin, or a declared
cgranges index without copying those resources into every resident transcript.
