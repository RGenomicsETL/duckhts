...
The following is the production shape, using the public GIAB HG002 v4.2.1 GRCh38
benchmark VCF, an Ensembl 116 GRCh38 model, dated ClinVar/ClinvArbitration,
AlphaMissense v2, gnomAD v2.1.1 gene constraint, and an Ensembl regulatory
Parquet relation. The provider files are ordinary typed Parquet; equivalent
...
human_stage <- human_evidence[, c(
  "stage", "result_rows", "seconds"
), drop = FALSE]
human_stage$stage <- c(
...
human_stage$result_rows <- format(
  human_stage$result_rows, big.mark = ",", scientific = FALSE
)
names(human_stage) <- c("stage", "result rows", "seconds")
```

The checked real-data smoke retained
`r format(unique(human_evidence$input_alleles), big.mark = ",")` chromosome-22
alleles and materialized
...
unpinned integration pass, so it validates composition and materialized output
rather than replacing the pinned performance tables below.

```{r duckvep-human-stage-table, echo=FALSE}
knitr::kable(human_stage, row.names = FALSE)
```
...
On the complete GIAB HG002 v4.2.1 input, `r format_count(unique(surface_evidence$variants))`
model-addressable literal alleles staged from
`r format_count(unique(surface_evidence$corpus_source_alt_count))` ALT alleles emit
`r format_count(unique(surface_evidence$annotated_rows))` annotation rows against the
complete Ensembl 116 GRCh38 model, including all
`r format_count(unique(surface_evidence$regulation_features))` resident regulatory and
motif features. At a 5,000-base transcript window, the checked public relation records
`r surface_rate("compact", 1L)` compact, `r surface_rate("rich", 1L)` rich,
`r surface_rate("hgvs", 1L)` HGVS, and `r surface_rate("rich_hgvs", 1L)` fused
rich-plus-HGVS alleles/s on one pinned physical core. Four pinned physical cores record
`r surface_rate("compact", 4L)`, `r surface_rate("rich", 4L)`,
`r surface_rate("hgvs", 4L)`, and `r surface_rate("rich_hgvs", 4L)` alleles/s
respectively. Input staging and model load are outside this resident-engine measurement;
every pass checksum-validates the complete output relation, and a separate full-row
fingerprint proves one-thread/four-thread equality for every output contract. The exact
revision (`r substr(surface_revision, 1L, 8L)`), hashes, core affinity, and workload are in
[benchmarks/duckvep_throughput.md](benchmarks/duckvep_throughput.md).

The separate end-to-end comparison includes VCF decoding, DuckVEP's explicit
coordinate sort, annotation, and real uncompressed tabular output. On one pinned
core DuckVEP completed in `r sprintf("%.2f", duck_one)` seconds versus
`r sprintf("%.2f", fast_one)` seconds for the current
native-compiled [FastVEP](https://github.com/Huang-lab/fastVEP) checkout
(`r sprintf("%.2f", fast_one / duck_one)`-fold wall-time ratio); at four pinned
cores the measured times were `r sprintf("%.2f", duck_four)` and
`r sprintf("%.2f", fast_four)` seconds
(`r sprintf("%.2f", fast_four / duck_four)`-fold). The tools emit different native tab schemas, so the
report preserves row/byte denominators and separately compares both against VEP
rather than presenting this as an isolated-kernel speedup. Build flags, source
commits, rebuilt cache receipt, commands, and output hashes are in
