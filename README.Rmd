...
This harness uses a Docker-only build path via `scripts/docker/duckdb-wasm-local.Dockerfile`.

The container pre-installs cache-friendly, pinned wasm build/runtime dependencies (`emsdk`, `vcpkg`, and duckdb-wasm runtime assets) so repeated local runs do minimal setup work.
...
The script stages a local copy of duckdb-wasm runtime assets (`duckdb-browser.mjs`, `duckdb-browser-eh.worker.js`, `duckdb-eh.wasm`) under `/duckdb-wasm-runtime/`, avoiding cross-origin worker restrictions from CDN-hosted worker scripts.
