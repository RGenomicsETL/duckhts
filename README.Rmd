...
- `region := 'r1,r2,...'` (comma-separated) is supported for `read_bam`, `read_bcf`, `read_gff`, `read_gtf`, and `read_tabix`.
- `read_bam`/`read_cram` region lists use htslib multi-region iterators and deduplicate overlapping regions; `read_bcf`/`read_gff`/`read_gtf`/`read_tabix` chain single-region iterators, so overlapping regions may duplicate rows.
