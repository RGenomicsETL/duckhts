...
If your host does not have `emcmake`, the script auto-builds and uses the project-owned Docker image at `scripts/docker/duckdb-wasm-local.Dockerfile`, which installs the same core wasm build pieces used by extension-ci-tools locally (`emsdk`, `vcpkg`, CMake/Ninja).
