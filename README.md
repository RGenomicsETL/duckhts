...
| `duckvep_haplotypes`                  | table       | TABLE(transcript_index UINTEGER, cds VARCHAR, protein VARCHAR, sequence_flags UINTEGER, evidence_flags UTINYINT, projection_status VARCHAR, sequence_status VARCHAR, edit_count UBIGINT, carrier_count UINTEGER, carriers STRUCT(sample_index UINTEGER, phase_set BIGINT, haplotype_lane USMALLINT, ploidy USMALLINT)\[\], contributors STRUCT(event_index UBIGINT, seq_region UINTEGER, position UBIGINT, reference VARCHAR, alternate VARCHAR, evidence_flags UTINYINT, projection_status VARCHAR)\[\], coding_blocks STRUCT(cds_start UINTEGER, reference VARCHAR, alternate VARCHAR, alt_start0 UBIGINT, length_change BIGINT, sequence_flags UINTEGER, event_indices UBIGINT\[\])\[\], cds_differences STRUCT(ref_start0 UBIGINT, alt_start0 UBIGINT, reference VARCHAR, alternate VARCHAR, alignment_start0 UBIGINT)\[\], protein_differences STRUCT(ref_start0 UBIGINT, alt_start0 UBIGINT, reference VARCHAR, alternate VARCHAR, alignment_start0 UBIGINT)\[\], stop_in_displaced_frame BOOLEAN) | `rduckhts_haplotypes` | Replay literal transcript haplotypes from an explicit event-by-transcript-by-sample SELECT query. stop_in_displaced_frame is BOOLEAN: TRUE when any base of the first translated stop codon overlaps a frame-displaced span of the rebuilt CDS, FALSE for no stop or a stop after restoration, and NULL when sequence is unavailable. A span starts at a frame-changing edit and ends after the restoring edit’s alternate bases, or continues downstream if unrestored. This fact does not change raw frame flags, remove later contributors, establish protein rescue, or classify combined SO. Required named columns are event_index UBIGINT (unique identity of one source ALT event), seq_region UINTEGER (model region ordinal), position UBIGINT (one-based), reference VARCHAR, alternate VARCHAR, alt_index UINTEGER (positive source ALT ordinal), transcript_index UINTEGER (selected candidate in the named model), sample_index UINTEGER, alleles INTEGER\[\] (NULL items are missing), phase_before BOOLEAN\[\] (NULL means unavailable), and nullable phase_set BIGINT. DuckDB casts these fields, derives complete sample/transcript phase-set domains and sorts by region, position, event, transcript and sample; candidate selection and source-record/ALT mapping remain explicit input relations. Duplicate calls, inconsistent event geometry/ALT identity, changed sample/transcript ploidy, invalid GT and out-of-model candidates fail. One query-owned native workspace retains the active genomic window, projects each event/candidate once, shares evidence-qualified carrier prefixes, and rebuilds/translates each occupied path once. Results expose transcript_index, cds, protein, sequence_flags, evidence_flags, projection_status, sequence_status, edit_count, carrier_count, carriers (sample_index, nullable phase_set, haplotype_lane, ploidy), and contributors (event_index, region, position, raw alleles, evidence_flags, projection_status). coding_blocks contains composite reference/alternate spans in ascending reference CDS order: cds_start is one-based in the reference CDS (insertion before that base), alt_start0 is zero-based in the rebuilt CDS, length_change is alternate minus reference length, sequence_flags records block indel/frame state, and event_indices lists one source event_index per physical edit in ascending reference CDS order. Repeated IDs preserve multiple differing islands from one uploaded MNV, including islands in distinct blocks. List length counts physical edits; join IDs to contributors for source provenance. Same-alternate-codon edits and frame-displacing/restoring edits share a block. Spans retain unchanged bases between edits and use transcript orientation; they are not an alignment, normalized HGVS or combined consequences. Insertions have empty reference and deletions empty alternate. Unknown/failed sequences have NULL blocks. Block and physical-edit provenance storage are bounded by max_leaf_edits within workspace_limit. cds_differences separately exposes aligned differing runs as ref_start0, alt_start0, reference, alternate and alignment_start0. All three positions are zero-based; reference/alternate are ungapped spans, empty on the gap side. The reference uses replay’s uppercase DNA spelling, without mutating the model. Adjacent runs join only for the same gap/non-gap type on both sides. Indel-bearing paths use VEP-116 pure-Perl global NW scoring (match +1, mismatch -1, gap -1) with reference-deletion, alternate-insertion, diagonal tie preference; substitution-only paths compare corresponding positions. This does not emulate Bio::Ext::Align. Repeat-associated alignment positions do not reassign event provenance and are not HGVS normalization. Unknown sequences have NULL differences. max_alignment_cells bounds the exact traceback band and max_leaf_differences bounds the number of runs. Both apply separately to each sequence axis within workspace_limit and fail explicitly without approximate alignment or omitted runs. protein_differences uses the same span schema and alignment mode, with positions in amino acids. Its reference follows Ensembl-116 consensus translation, removal of the last complete translated stop, legitimate-start methionine and the model’s single-residue peptide edits. Haplosaurus then appends ’\*’ only for an exact uppercase TAA/TAG/TGA raw-CDS suffix; this convention is retained for nonstandard tables, partial CDS and lowercase model bytes. The alternate protein is the displayed first-stop prefix without reference peptide edits. Protein differences are NULL for unknown paths or a reference CDS with no complete codon, and empty for known identical proteins. Reference preparation is cached once per closing transcript in worker storage. CDS and protein differences reuse the same bounded traceback and descriptors, with no model mutation or per-row allocation. Evidence bits are 1 called, 2 missing, 4 unphased. Missing/unphased paths preserve provenance but have NULL sequence and incomplete_input status. Strict policy respects per-allele phase and PS; homozygous/haploid calls reach later phase sets. vep116_compat ignores PS/separators and compacts called slots, but missing paths remain incomplete: raw mixed-separator parsing, upstream missing-call conditional sequences and ploidy inference are not certified. This alpha sequence-mechanics surface does not yet emit combined SO, compound HGVS or typed structural composition. Each per-call capacity bounds its named active pool, leaf, sequence, genotype or phase domain; workspace_limit caps the sum of native allocations, excluding DuckDB-owned input/sort/output memory. Exhaustion errors rather than grows or drops rows. Input is materialized on the registry’s retained connection; caller-local TEMP objects and uncommitted changes are not visible. A single query-preparation slot is shared with model loading: nested/concurrent preparation returns a busy error and concurrent callers may retry; completed scans have independent state. Return order and carrier-list order are not SQL ordering guarantees. Join event/model/sample ordinals to retained cold relations for stable biological identifiers and provenance. |
| `duckvep_phase_call`                  | scalar      | STRUCT(input_slot USMALLINT, allele_index INTEGER, haplotype_lane USMALLINT, ploidy USMALLINT, phase_set BIGINT, phase_scope VARCHAR, status VARCHAR)\[\]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |                       | Prepare one typed GT call for phased execution without parsing genotype strings or allocating native scratch. Consume read_geno calls.alleles and calls.phase_before before ALT expansion; retain record/sample/model identity and raw GT/PS beside the result. One output entry preserves each one-based input_slot, source allele_index (0 REF, positive source ALT ordinal, NULL missing) and ploidy, limited to 1..65535. A NULL GT returns NULL, not reference or an empty carrier list. NULL phase list or flags mean unavailable phase; otherwise lists must have equal length. Empty GT lists and negative called allele indices fail. strict preserves explicitly phased lanes and resolves remaining unphased slots only if their assignment cannot change under permutation. Homozygous called GTs and haploid calls have all_phase_sets scope: their facts apply across phase sets, not merely the NULL-PS set. phase_set scope retains the supplied signed BIGINT identity; NULL denotes the default set and differs from present zero. The input is decoded data, not VCF PS schema validation. Scope is always local to the caller’s sample and chromosome. Unresolved heterozygous slots have NULL haplotype_lane, unresolved scope and unphased status; missing alleles keep missing status even when a lane is known. vep116_compat ignores separators/PS and assigns allele_slot lanes after removing missing entries, matching the pinned VEP parser’s input to Haplosaurus. Missing input entries remain explicit with NULL lane. Decoded ploidy is preserved; this helper does not emulate VEP’s raw mixed/prefixed-separator parsing or its container’s ploidy inference. NULL policy means strict; unknown policy fails. Do not discard missing/unresolved entries or treat all_phase_sets as a separate ordinary PS when constructing haplotypes. This GT/PS-only helper does not interpret PSL/PSO/PID, phase records, verify ALT bounds against a source record, build haplotypes, or emit combined SO/HGVS. Only use it with a declared GT/PS phasing source; alternative phase encodings require their own explicit adapter.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
