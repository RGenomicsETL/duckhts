...
run_convert_parquet_sql <- function(
  ...
  if (is.na(output_exists) && !overwrite) {
    stop(
      "Could not check output path with DuckDB: ",
      output,
...
convert_parquet_common_params <- function(
  ...
  row_group_size <- .validate_nonnegative_integer_param(
    row_group_size,
    "row_group_size"
  )
  ...
  if (
    !is.logical(include_metadata) ||
      length(include_metadata) != 1L ||
  ...
  if (!is.null(header_text)) {
    if (
      !is.character(header_text) ||
        length(header_text) == 0L ||
        anyNA(header_text)
    ) {
      stop(
        "header_text must be a character string or character vector without NA values",
        call. = FALSE
      )
    }
  ...
  if (!is.null(metadata_json_file)) {
    if (
      !is.character(metadata_json_file) ||
        length(metadata_json_file) != 1L ||
        is.na(metadata_json_file) ||
        !nzchar(metadata_json_file)
    ) {
      stop(
        "metadata_json_file must be a single non-empty character string",
        call. = FALSE
      )
    }
  }
  ...
  if (
    !is.character(write_format_version) ||
      length(write_format_version) != 1L ||
      is.na(write_format_version) ||
      !nzchar(write_format_version)
  ) {
    stop(
      "write_format_version must be a single non-empty character string",
      call. = FALSE
    )
  }
  ...
  if (!is.null(metadata)) {
    if (is.null(names(metadata)) || any(!nzchar(names(metadata)))) {
      stop(
        "metadata must be a named list or named character vector",
        call. = FALSE
      )
    }
    ...
    metadata <- vapply(
      ...
      function(key) {
        ...
        if (anyNA(value)) {
          stop("metadata value for '", key, "' must not be NA", call. = FALSE)
        }
...
rduckhts_bcf_convert_parquet <- function(
  ...
  if (
    !is.logical(tidy_format) || length(tidy_format) != 1L || is.na(tidy_format)
  ) {
...
rduckhts_bam_convert_parquet <- function(
  ...
  if (
    !is.logical(standard_tags) ||
      length(standard_tags) != 1L ||
      is.na(standard_tags)
  ) {
    stop("standard_tags must be TRUE or FALSE", call. = FALSE)
  }
  if (
    !is.logical(auxiliary_tags) ||
      length(auxiliary_tags) != 1L ||
      is.na(auxiliary_tags)
  ) {
...
rduckhts_gff_convert_parquet <- function(
  ...
  if (
    !is.logical(attributes_list) ||
      length(attributes_list) != 1L ||
      is.na(attributes_list)
  ) {
    stop("attributes_list must be TRUE or FALSE", call. = FALSE)
  }
  if (
    !is.logical(attributes_pairs) ||
      length(attributes_pairs) != 1L ||
      is.na(attributes_pairs)
  ) {
