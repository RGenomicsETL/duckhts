/*
 * duckvep_sweep.h — candidate (event × transcript) generation via a sorted,
 * resumable multi-track sweep. INTERNAL: exposed for white-box property tests.
 *
 * Preconditions:
 *   - events sorted ascending by (chrom_id, pos1/start1);
 *   - transcripts sorted ascending by (chrom_id, start1).
 * A transcript T is a candidate for event E iff, on the same chromosome,
 *
 ...
 * frontier at raw E.start1+halo, then appends the sorted transcript tail whose starts
 ...
 * transcript on the chromosome. `seed` is the unique set that overlaps the
...
/* Advance to the next event and expose its exact candidate transcript slice.
