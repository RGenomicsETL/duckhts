...
duckvep_register_ensembl_transcripts(duckdb_connection connection)
{
	static const char *const sql[] = {
		...
		"max(ta.value) FILTER (WHERE aty.code = 'MANE_Select') AS mane_select_refseq, ",
		"max(ta.value) FILTER (WHERE aty.code = 'MANE_Plus_Clinical') AS mane_plus_clinical_refseq, ",
		"count(DISTINCT ta.value) FILTER (WHERE aty.code = 'MANE_Select') AS mane_select_refseq_count, ",
		"count(DISTINCT ta.value) FILTER (WHERE aty.code = 'MANE_Plus_Clinical') AS mane_plus_clinical_refseq_count, ",
		...
		"SELECT ta.transcript_id AS source_transcript_id, ta.value, ",
		"try_cast(regexp_extract(ta.value, '([0-9]+)-([0-9]+)', 1) AS UBIGINT) AS cdna_start, ",
		"try_cast(regexp_extract(ta.value, '([0-9]+)-([0-9]+)', 2) AS UBIGINT) AS cdna_end, ",
		...
		"AS mature_mirna_regions, ts.cdna_sequence, ",
		...
		"coalesce(p.attribute_withheld_reason, CASE WHEN p.raw_cds_sequence IS NOT NULL AND ",
		"(NOT regexp_full_match(p.raw_cds_sequence, '[ACGTN]+') ",
		"OR NOT regexp_full_match(p.raw_pre_cds_sequence, '[ACGTN]*') ",
		"OR NOT regexp_full_match(p.raw_post_cds_sequence, '[ACGTN]*')) ",
		"THEN 'unsupported_reference_base' END) AS sequence_withheld_reason, ",
		"p.exons, p.mature_mirna_regions FROM prepared p CROSS JOIN validation ",
...
duckvep_register_model_receipt(duckdb_connection connection)
{
	static const char *const sql[] = {
		...
		"exons := exons, mature_mirna_regions := mature_mirna_regions) AS VARCHAR)) ",
		...
		"summary.transcript_flank_base_count ",
