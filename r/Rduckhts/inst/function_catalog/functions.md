...
| Function | Kind | Returns | R helper | Description |
| --- | --- | --- | --- | --- |
| `bcftools_liftover` | scalar | STRUCT | `rduckhts_score` | Score-row-oriented liftover kernel intended to mirror bcftools +liftover semantics as closely as possible while returning one STRUCT per input row with source fields, lifted coordinates/alleles, reverse-complement state, swap flag, reject_reason for rejected rows, and note annotations for emitted rows that need extra interpretation. Set no_left_align := true to skip left-alignment of lifted indels (mirrors --no-left-align in bcftools +liftover). |
