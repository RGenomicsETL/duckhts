...
test_score <- function() {
  ...
  out_bundled_list <- rduckhts_score(
    con,
    vcf,
    summary_path = NULL,
  ...
  out_log <- rduckhts_score(
    con,
    vcf,
    c(sumf, sumf_mismatch),
    use = "GT",
    columns = "PLINK",
    log_path = log_path
  )
  ...
  expect_true(all(
    c(
      ...
      "loaded_markers",
      "matched_markers",
      "allele_mismatch_markers"
    ) %in%
      names(log_tbl)
  ))
  mismatch_row <- log_tbl[
    log_tbl$summary_name == "score_summary_mismatch",
    ,
    drop = FALSE
  ]
  ...
  expect_true(all(
    c(
      "score_summary_p0.01",
      "score_summary_CNT_p0.01",
      "score_summary_CNT_p0.2"
    ) %in%
      names(out_thr)
  ))
  ...
  out_regions_file <- rduckhts_score(
    con,
    vcf,
    sumf,
    use = "GT",
  ...
  out_targets_file <- rduckhts_score(
    con,
    vcf,
    sumf,
    use = "GT",
  ...
  out_filter_expr <- rduckhts_score(
    con,
    vcf,
    sumf,
    use = "GT",
  ...
  expect_true(all(
    c("PRS_A", "PRS_A_CNT", "PRS_B", "PRS_B_CNT") %in% names(out_gwas_cnt)
  ))
  ...
  expect_true(all(
    c("PRS_A_p0.1", "PRS_A_CNT_p0.1", "PRS_B_p0.1", "PRS_B_CNT_p0.1") %in%
      names(out_gwas_thr)
  ))
