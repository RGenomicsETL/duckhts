...
test_seq_ops <- function() {
  ...
  expect_silent(rduckhts_fasta_index(
    con,
    fasta_path,
  ...
  forced <- DBI::dbGetQuery(
    con,
    "SELECT backend AS selected FROM duckhts_simd_set_backend('scalar')"
  )
  expect_identical(forced$selected[1], "scalar")
  auto <- DBI::dbGetQuery(
    con,
    "SELECT backend AS selected FROM duckhts_simd_set_backend('auto')"
  )
  expect_true(nzchar(auto$selected[1]))
  auto_req <- DBI::dbGetQuery(
    con,
    "SELECT duckhts_simd_requested_backend() AS requested"
  )
  ...
  expect_true(all(
    simd_info$available == (simd_info$compiled & simd_info$cpu_supported)
  ))
  ...
  expect_identical(rduckhts_simd_set_backend(con, "scalar"), "scalar")
  gc_nt16_scalar <- DBI::dbGetQuery(
    con,
    "SELECT seq_gc_content(seq_encode_4bit(repeat('CGAT', 16))) AS gc"
  )$gc[1]
  bam_gc_sql <- sprintf(
    paste(
  ...
  for (backend in complete_backends) {
    ...
    concrete_rows <- concrete_info[
      concrete_info$kernel %in%
        c(
          "seq_base_counts",
          "bam_nt16_counts",
          "nt16_gc_counts",
    ...
    gc_nt16_concrete <- DBI::dbGetQuery(
      con,
      "SELECT seq_gc_content(seq_encode_4bit(repeat('CGAT', 16))) AS gc"
    )$gc[1]
    expect_equal(gc_nt16_concrete, gc_nt16_scalar, info = backend)
    expect_equal(
      DBI::dbGetQuery(con, bam_gc_sql),
      bam_gc_scalar,
      info = backend
    )
  }
  ...
  expect_error(
    rduckhts_simd_backend_available(con, character()),
    "single non-missing"
  )
  expect_error(
    rduckhts_simd_set_backend(con, c("scalar", "auto")),
    "single non-missing"
  )
  expect_error(
    rduckhts_simd_set_backend(con, NA_character_),
    "single non-missing"
  )
  ...
  expect_true(nzchar(rduckhts_simd_set_backend(con, "auto")))
  expect_error(
    rduckhts_simd_set_backend(con, "sse2"),
    "no selectable implementation"
  )
  expect_error(
    rduckhts_simd_set_backend(con, "not-a-backend"),
    "unknown SIMD backend"
  )
  ...
  sql_vals <- function(xs) {
    paste(sprintf("seq_gc_content('%s')", xs), collapse = ", ")
  }

  expect_identical(rduckhts_simd_set_backend(con, "scalar"), "scalar")
  scalar_kernel_info <- rduckhts_simd_kernel_info(con)
  expect_identical(
    scalar_kernel_info$selected_backend[
      scalar_kernel_info$kernel == "seq_base_counts"
    ],
    "scalar"
  )
  expect_false(scalar_kernel_info$scalar_fallback[
    scalar_kernel_info$kernel == "seq_base_counts"
  ])
  ...
  bam_str_type <- DBI::dbGetQuery(
    con,
    "SELECT typeof(SEQ) AS t FROM bam_str LIMIT 1"
  )
  ...
  bam_threads_off_n <- DBI::dbGetQuery(
    con,
    "SELECT COUNT(*) AS n FROM bam_threads_off"
  )
  ...
  bam_nt16_type <- DBI::dbGetQuery(
    con,
    "SELECT typeof(SEQ) AS t FROM bam_nt16 LIMIT 1"
  )
  expect_equal(bam_nt16_type$t[1], "UTINYINT[]")

  # first 5 codes match expected AGCTA -> [1, 4, 2, 8, 1]
  bam_codes <- DBI::dbGetQuery(
    con,
  ...
  on.exit(
    unlink(
      c(gz_fasta_path, gz_fasta_index_path, custom_gzi_path),
      force = TRUE
    ),
    add = TRUE
  )
  ...
  expect_error(DBI::dbGetQuery(
    con,
    sprintf(
      "SELECT * FROM read_fasta('%s', sequence_encoding := 'bad')",
      fasta_path
    )
  ))
  ...
  fq_multiline_packed <- DBI::dbGetQuery(
    con,
    sprintf(
      paste(
  ...
  expect_error(
    DBI::dbGetQuery(
      con,
      sprintf(
        "SELECT * FROM read_fastq('%s')",
        fastq_truncated
      )
    ),
    pattern = "truncated FASTQ quality block",
    fixed = TRUE
  )
  ...
  expect_error(DBI::dbGetQuery(
    con,
    sprintf(
      "SELECT * FROM read_fastq('%s', sequence_encoding := 'xyz')",
      fastq_r1
    )
  ))
