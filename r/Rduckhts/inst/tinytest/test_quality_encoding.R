...
test_quality_encoding <- function() {
  ...
  expect_true(grepl(
    "phred64",
    legacy_detect$compatible_encodings[1],
    fixed = TRUE
  ))
  ...
  legacy_q_auto <- DBI::dbGetQuery(
    con,
    "SELECT QUALITY FROM legacy_fq_auto LIMIT 1"
  )
