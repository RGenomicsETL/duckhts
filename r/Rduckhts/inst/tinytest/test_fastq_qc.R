...
test_fastq_qc <- function() {
  ...
  long_qname_summary <- DBI::dbGetQuery(
    con,
    sprintf(
      paste(
        "WITH q AS (",
  ...
  expect_error(
    DBI::dbGetQuery(
      con,
      sprintf("SELECT NAME FROM read_fastq(%s)", quoted_long_qname_fastq)
    ),
  ...
  single_cycle_limit <- DBI::dbGetQuery(
    con,
    paste(
