...
with_duckhts_con({
  ...
  rows <- DBI::dbGetQuery(
    con,
    sprintf("SELECT count(*) AS n FROM read_parquet(%s)", qstr(out))
  )$n
...
with_duckhts_con({
  ...
  expect_true(
    DBI::dbGetQuery(
      con,
      sprintf("SELECT count(*) > 0 AS ok FROM read_parquet(%s)", qstr(out))
    )$ok
  )
})
...
with_duckhts_con({
  ...
  expect_true(
    DBI::dbGetQuery(
      con,
      sprintf(
        "SELECT count(*) > 0 AS ok FROM read_parquet(%s, hive_partitioning = true)",
        qstr(file.path(out_dir, "**", "*.parquet"))
      )
    )$ok
  )
})
...
with_duckhts_con({
  ...
  expect_identical(
    DBI::dbGetQuery(
      con,
      sprintf(
        "SELECT typeof(pos) AS type FROM read_parquet(%s) LIMIT 1",
        qstr(out)
      )
    )$type,
    "BIGINT"
