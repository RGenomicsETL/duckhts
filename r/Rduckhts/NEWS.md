...
- fix bundled `rduckhts_liftover()` / `bcftools_liftover(...)` repeated-sequence indel parity in the upstream allele-extension path: repeat-run insertions now keep extending across the source reference when needed, restoring upstream-compatible bundled `dest_ref` / `dest_alt` / `swap` results for cases such as `G>GT` insertions across destination-side repeat expansions; bundled SQL/tinytest coverage now includes a dedicated repeat-run regression fixture and the real-data HG001 chr20 conformance workflow reaches exact mapped-row parity with installed `bcftools +liftover`
