...
- fix bundled `rduckhts_liftover()` / `bcftools_liftover(...)` FASTA contig alias handling during source/destination reference validation and sequence fetches: inputs such as `23`, `24`, `26`, `X`, `Y`, `MT`, and `chr*` aliases now resolve through the same canonical path, avoiding spurious `SourceRefMismatch` rejects for X/Y/MT indels when the bundled chain names and FASTA names differ only by canonical aliasing; bundled SQL/tinytest coverage now pins the `23 -> chrX` regression
