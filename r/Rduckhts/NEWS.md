...
- extend bundled `rduckhts_score()` / `bcftools_score(...)` so `summary_path` can be a character vector or callers can use `summaries_list_file`; multiple TSV/SSF summaries are scored in one genotype scan, and `log_path` can write per-PRS matching/audit counts for loaded, matched, allele-mismatch, and duplicate markers
