...
- Speed up bundled `read_fastq(...)` / `rduckhts_fastq()` `COUNT(*)` queries: when DuckDB projects zero FASTQ columns, the extension now counts records directly from the raw FASTQ stream instead of materializing each row through `sam_read1()`, while preserving paired/interleaved validation errors.
