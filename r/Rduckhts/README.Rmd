...
`read_bam(...)`, `read_bcf(...)`, or regular tables. For one-row-per-hit
expansion over a larger probe set, `duckhts_cgranges_overlaps_bulk(...)` accepts
a probe query and emits matching indexed intervals in one table-function call.
That bulk query runs on the extension-owned helper connection, so use a regular
table or view rather than a temp table.
