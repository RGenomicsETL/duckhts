...
duckhts_bench_stage_liftover <- function(
  ...
  if (!file.exists(source_fasta) || !file.info(source_fasta)$size) {
    temporary <- paste0(source_fasta, ".partial")
  ...
  for (fasta in c(source_fasta, destination_fasta)) {
    if (!file.exists(paste0(fasta, ".fai")) || !file.info(paste0(fasta, ".fai"))$size) {
      ...
      if (status != 0L) stop("could not index liftover FASTA: ", fasta, call. = FALSE)
    }
  }
